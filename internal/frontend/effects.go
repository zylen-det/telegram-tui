...
type Handler struct {
	...
	draftPending map[draftKey]draftSaveWork
	draftRunning map[draftKey]bool

	// updates carries TelegramEvent values and the terminal stream notice from
	// the client pump goroutine to the single Bubble Tea subscription. Exactly
	// one waitForUpdate command is outstanding at a time.
	updates chan Event
}
...
func NewHandler(processCtx context.Context, resolver RuntimeResolver, factory ClientFactory, prompts *auth.Broker, avatars AvatarRenderer, clipboard ...platform.Clipboard) *Handler {
	...
	handler := &Handler{
		...
		draftPending: make(map[draftKey]draftSaveWork),
		draftRunning: make(map[draftKey]bool),
...
func (h *Handler) dispatch(command Effect, collect func(Event)) {
	switch command := command.(type) {
	case LoadBootstrap:
		if failure := h.start(); failure != nil {
			collect(failure)
		}
	case LoadChats:
		client, err := h.currentClient("load chats")
		if err != nil {
			collect(ChatsLoadFailed{RequestID: command.RequestID, Error: *err})
			return
		}
		page, loadErr := client.LoadChats(h.workCtx, command.Cursor)
		if loadErr != nil {
			collect(ChatsLoadFailed{RequestID: command.RequestID, Error: safeError("load chats", loadErr)})
			return
		}
		collect(ChatsLoaded{RequestID: command.RequestID, Page: page})
	case LoadMessages:
		client, err := h.currentClient("load messages")
		if err != nil {
			collect(MessagesLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, Error: *err})
			return
		}
		...
		if loadErr != nil {
			collect(MessagesLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, Error: safeError("load messages", loadErr)})
			return
		}
		collect(MessagesLoaded{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, Page: page})
	case LoadTopics:
		client, err := h.currentClient("load topics")
		if err != nil {
			collect(TopicsLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, Error: topicsLoadError()})
			return
		}
		...
		collect(TopicsLoaded{RequestID: command.RequestID, ChatID: command.ChatID, Page: page})
	case SearchChatMessages:
		client, err := h.currentClient("search messages")
		if err != nil {
			collect(ChatMessagesSearchFailed{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, Error: messageSearchError()})
			return
		}
		cursor := command.Cursor
		cursor.TopicID = command.TopicID
		...
		collect(ChatMessagesSearched{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, Page: page})
	case SearchPublicChatCommand:
		client, err := h.currentClient("search public chat")
		if err != nil {
			collect(PublicChatSearchFailed{RequestID: command.RequestID, Error: chatSearchError()})
			return
		}
		...
		collect(PublicChatSearched{RequestID: command.RequestID, Chat: chat})
	case SearchPublicChatsCommand:
		client, err := h.currentClient("search public chats")
		if err != nil {
			collect(PublicChatsSearchFailed{RequestID: command.RequestID, Error: domain.AppError{Kind: domain.ErrorNetwork, Op: "search public chats", Message: "Could not search public chats"}})
			return
		}
		...
		collect(PublicChatsSearched{RequestID: command.RequestID, Chats: chats})
	case SearchAllMessagesCommand:
		client, err := h.currentClient("search all messages")
		if err != nil {
			collect(AllMessagesSearchFailed{RequestID: command.RequestID, Error: domain.AppError{Kind: domain.ErrorNetwork, Op: "search all messages", Message: "Could not search messages"}})
			return
		}
		...
		collect(AllMessagesSearched{RequestID: command.RequestID, Messages: page.Messages, TotalCount: page.TotalCount})
	case LoadMembers:
		client, err := h.currentClient("load members")
		if err != nil {
			collect(MembersLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, Error: membersError()})
			return
		}
		...
		collect(MembersLoaded{RequestID: command.RequestID, ChatID: command.ChatID, Page: page})
	case LoadInviteLinksCommand:
		client, err := h.currentClient("load invite links")
		if err != nil {
			collect(InviteLinksLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID})
			return
		}
		...
		collect(InviteLinksLoaded{RequestID: command.RequestID, ChatID: command.ChatID, Page: page})
	case CreateInviteLinkCommand:
		client, err := h.currentClient("create invite link")
		if err != nil {
			collect(InviteLinkCreateFailed{RequestID: command.RequestID, ChatID: command.ChatID})
			return
		}
		linksClient, ok := client.(telegram.InviteLinksClient)
		if !ok {
		...
		collect(InviteLinkCreated{RequestID: command.RequestID, ChatID: command.ChatID, Link: link})
	case RevokeInviteLinkCommand:
		client, err := h.currentClient("revoke invite link")
		if err != nil {
			collect(InviteLinkRevokeFailed{RequestID: command.RequestID, ChatID: command.ChatID, URL: command.URL})
			return
		}
		linksClient, ok := client.(telegram.InviteLinksClient)
		...
		collect(InviteLinkRevoked{RequestID: command.RequestID, ChatID: command.ChatID, URL: command.URL})
	case CopyInviteLinkCommand:
		if h.clipboard == nil || h.clipboard.WriteText(h.workCtx, command.URL) != nil {
			collect(InviteLinkCopyFailed{RequestID: command.RequestID, ChatID: command.ChatID, URL: command.URL})
			return
		}
		collect(InviteLinkCopied{RequestID: command.RequestID, ChatID: command.ChatID, URL: command.URL})
	case LoadAdministrationCommand:
		client, err := h.currentClient("load administration")
		if err != nil {
			collect(AdministrationLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID})
			return
		}
		...
		collect(AdministrationLoaded{RequestID: command.RequestID, ChatID: command.ChatID, Snapshot: v})
	case LoadChatSettingsCommand:
		client, err := h.currentClient("load chat settings")
		if err != nil {
			collect(ChatSettingsLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID})
			return
		}
		...
		collect(ChatSettingsLoaded{RequestID: command.RequestID, ChatID: command.ChatID, Snapshot: v})
	case SaveChatSettingCommand:
		...
		if failed {
			collect(ChatSettingSaveFailed{RequestID: command.RequestID, ChatID: command.ChatID, Field: command.Field})
			return
		}
		collect(ChatSettingSaved{RequestID: command.RequestID, ChatID: command.ChatID, Field: command.Field})
	case LoadMemberAdministrationCommand:
		client, err := h.currentClient("load member administration")
		if err != nil {
			collect(MemberAdministrationLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID})
			return
		}
		c, ok := client.(telegram.AdministrationClient)
		if !ok {
		...
		collect(MemberAdministrationLoaded{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, Status: v})
	case SetDefaultChatPermissionsCommand:
		client, err := h.currentClient("save permissions")
		if err != nil {
			collect(DefaultPermissionsSaveFailed{RequestID: command.RequestID, ChatID: command.ChatID})
			return
		}
		c, ok := client.(telegram.AdministrationClient)
		...
		collect(DefaultPermissionsSaved{RequestID: command.RequestID, ChatID: command.ChatID})
	case ApplyMemberAdministrationCommand:
		client, err := h.currentClient("apply member administration")
		req := command.Request
		if err != nil {
			collect(MemberAdministrationApplyFailed{RequestID: command.RequestID, ChatID: req.ChatID, UserID: req.UserID, Action: req.Action})
			return
		}
		c, ok := client.(telegram.AdministrationClient)
		...
		collect(MemberAdministrationApplied{RequestID: command.RequestID, ChatID: req.ChatID, UserID: req.UserID, Action: req.Action})
	case LoadUserInfo:
		client, err := h.currentClient("load user")
		if err != nil {
			collect(UserInfoLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, Error: userInfoError()})
			return
		}
		...
		collect(UserInfoLoaded{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, User: user})
	case CopyMemberUsernameCommand:
		if h.clipboard == nil {
			collect(MemberUsernameCopyFailed{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, Error: memberCopyError()})
			return
		}
		...
		collect(MemberUsernameCopied{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID})
	case AddMemberContactCommand:
		client, err := h.currentClient("add contact")
		if err != nil {
			collect(MemberContactFailed{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, Added: true, Error: memberContactError(true)})
			return
		}
		...
		collect(MemberContactChanged{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, Added: true})
	case RemoveMemberContactCommand:
		client, err := h.currentClient("remove contact")
		if err != nil {
			collect(MemberContactFailed{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, Added: false, Error: memberContactError(false)})
			return
		}
		...
		collect(MemberContactChanged{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, Added: false})
	case SetMemberBlockedCommand:
		client, err := h.currentClient("block user")
		if err != nil {
			collect(MemberBlockFailed{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, Blocked: command.Blocked, Error: memberBlockError(command.Blocked)})
			return
		}
		...
		collect(MemberBlockChanged{RequestID: command.RequestID, ChatID: command.ChatID, UserID: command.UserID, Blocked: command.Blocked})
	case ApplyChatActionCommand:
		client, err := h.currentClient("update chat")
		if err != nil {
			collect(ChatActionFailed{RequestID: command.RequestID, ChatID: command.ChatID, Action: command.Action, Error: chatActionError(command.Action)})
			return
		}
		...
		collect(ChatActionApplied{RequestID: command.RequestID, ChatID: command.ChatID, Action: command.Action})
	case LoadPinnedMessages:
		client, err := h.currentClient("load pinned messages")
		if err != nil {
			collect(PinnedMessagesLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, Error: pinnedSearchError()})
			return
		}
		cursor := command.Cursor
		cursor.TopicID = command.TopicID
		...
		collect(PinnedMessagesLoaded{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, Page: page})
	case LoadPinnedMessageContext:
		client, err := h.currentClient("load pinned message context")
		if err != nil {
			collect(PinnedMessageContextFailed{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, MessageID: command.MessageID, Error: pinnedContextError()})
			return
		}
		var (
		...
		collect(PinnedMessageContextLoaded{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, MessageID: command.MessageID, Page: page})
	case LoadBotCommands:
		client, err := h.currentClient("load bot commands")
		if err != nil {
			collect(BotCommandsLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, Error: *err})
			return
		}
		commands, loadErr := client.LoadBotCommands(h.workCtx, command.ChatID)
		if loadErr != nil {
			collect(BotCommandsLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, Error: safeError("load bot commands", loadErr)})
			return
		}
		collect(BotCommandsLoaded{RequestID: command.RequestID, ChatID: command.ChatID, Commands: append([]domain.BotCommand(nil), commands...)})
	case LoadSearchMessageContext:
		client, err := h.currentClient("load message context")
		if err != nil {
			collect(SearchMessageContextFailed{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, MessageID: command.MessageID, Error: messageContextError()})
			return
		}
		var (
			page       telegram.MessagePage
		...
		collect(SearchMessageContextLoaded{RequestID: command.RequestID, ChatID: command.ChatID, TopicID: command.TopicID, MessageID: command.MessageID, Page: page})
	case LoadStickers:
		client, err := h.currentClient("load stickers")
		if err != nil {
			collect(StickersLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, Error: *err})
			return
		}
		stickers, loadErr := client.LoadStickers(h.workCtx)
		if loadErr != nil {
			collect(StickersLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, Error: safeError("load stickers", loadErr)})
			return
		}
		collect(StickersLoaded{RequestID: command.RequestID, ChatID: command.ChatID, Stickers: append([]domain.StickerRef(nil), stickers...)})
	case GetMessageProperties:
		client, err := h.currentClient("get message properties")
		if err != nil {
			collect(MessagePropertiesLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: messagePropertiesError(nil)})
			return
		}
		capabilities, loadErr := client.GetMessageProperties(h.workCtx, command.ChatID, command.MessageID)
		if loadErr != nil {
			collect(MessagePropertiesLoadFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: messagePropertiesError(loadErr)})
			return
		}
		collect(MessagePropertiesLoaded{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Capabilities: capabilities})
	case SaveDraft:
		h.saveDraft(command, collect)
	case SendText:
		client, err := h.currentClient("send text")
		if err != nil {
			collect(TextQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, Error: *err, FailedAt: h.now()})
			return
		}
		...
		if sendErr != nil {
			collect(TextQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, Error: safeError("send text", sendErr), FailedAt: h.now()})
			return
		}
		collect(TextQueued{RequestID: command.RequestID, LocalID: command.LocalID, Message: message})
	case SendPhoto:
		client, err := h.currentClient("send photo")
		if err != nil {
			collect(PhotoQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: *err, FailedAt: h.now()})
			return
		}
		...
		if sendErr != nil {
			collect(PhotoQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: safeError("send photo", sendErr), FailedAt: h.now()})
			return
		}
		collect(PhotoQueued{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Message: message})
	case SendVideo:
		client, err := h.currentClient("send video")
		if err != nil {
			collect(VideoQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: *err, FailedAt: h.now()})
			return
		}
		...
		if sendErr != nil {
			collect(VideoQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: safeError("send video", sendErr), FailedAt: h.now()})
			return
		}
		collect(VideoQueued{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Message: message})
	case SendSticker:
		client, err := h.currentClient("send sticker")
		if err != nil {
			collect(StickerQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: *err, FailedAt: h.now()})
			return
		}
		...
		if sendErr != nil {
			collect(StickerQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: safeError("send sticker", sendErr), FailedAt: h.now()})
			return
		}
		collect(StickerQueued{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Message: message})
	case SendAudio:
		client, err := h.currentClient("send audio")
		if err != nil {
			collect(AudioQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: *err, FailedAt: h.now()})
			return
		}
		...
		if sendErr != nil {
			collect(AudioQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: safeError("send audio", sendErr), FailedAt: h.now()})
			return
		}
		collect(AudioQueued{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Message: message})
	case SendDocument:
		client, err := h.currentClient("send document")
		if err != nil {
			collect(DocumentQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: documentSendError(*err), FailedAt: h.now()})
			return
		}
		...
		if sendErr != nil {
			collect(DocumentQueueFailed{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Error: documentSendError(sendErr), FailedAt: h.now()})
			return
		}
		collect(DocumentQueued{RequestID: command.RequestID, LocalID: command.LocalID, ChatID: command.ChatID, Message: message})
	case EditText:
		client, err := h.currentClient("edit text")
		if err != nil {
			collect(TextEditFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: editTextError(nil)})
			return
		}
		...
		if editErr != nil {
			collect(TextEditFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: editTextError(editErr)})
			return
		}
		collect(TextEdited{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Message: message})
	case DeleteMessageCommand:
		client, err := h.currentClient("delete message")
		if err != nil {
			collect(MessageDeleteFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: deleteError(nil)})
			return
		}
		...
		if deleteErr != nil {
			collect(MessageDeleteFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: deleteError(deleteErr)})
			return
		}
		collect(MessageDeleted{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID})
	case PinMessageCommand:
		client, err := h.currentClient("pin message")
		if err != nil {
			collect(MessagePinFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: pinError(nil)})
			return
		}
		...
		if pinErr != nil {
			collect(MessagePinFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: pinError(pinErr)})
			return
		}
		collect(MessagePinChanged{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Pinned: !command.Unpin})
	case ReactToMessage:
		client, err := h.currentClient("react to message")
		if err != nil {
			collect(ReactionFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: reactError(err)})
			return
		}
		...
		if reactErr != nil {
			collect(ReactionFailed{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Error: reactError(reactErr)})
			return
		}
		collect(ReactionChanged{RequestID: command.RequestID, ChatID: command.ChatID, MessageID: command.MessageID, Emoji: command.Emoji, Removed: command.Remove})
	case ForwardMessageCommand:
		client, err := h.currentClient("forward message")
		if err != nil {
			collect(MessageForwardFailed{RequestID: command.RequestID, DestinationChatID: command.DestinationChatID, Error: forwardError(nil)})
			return
		}
		...
		if forwardErr != nil {
			collect(MessageForwardFailed{RequestID: command.RequestID, DestinationChatID: command.DestinationChatID, Error: forwardError(forwardErr)})
			return
		}
		collect(MessageForwarded{RequestID: command.RequestID, DestinationChatID: command.DestinationChatID})
	case OpenChatCommand:
		h.openChat(command)
	case CloseChatCommand:
		h.closeChat(command)
	case RenderAvatar:
		h.renderAvatar(command, collect)
	case OpenAvatar:
		h.openAvatar(command, collect)
	case OpenMessageMediaFile:
		h.openMessageMedia(command, collect)
	case DownloadStickerThumbnail:
		h.downloadStickerThumbnail(command, collect)
	case DownloadThumbnail:
		h.downloadThumbnail(command, collect)
	case SubmitPrompt:
		if h.prompts == nil {
			collect(OperationFailed{Error: safeError("submit authorization prompt", errors.New("prompt broker unavailable"))})
			return
		}
		if err := h.prompts.Submit(h.workCtx, command.Response); err != nil {
			collect(OperationFailed{Error: safeError("submit authorization prompt", err)})
		}
	case BeginShutdown:
	...
	default:
		collect(OperationFailed{Error: safeError("handle command", errors.New("unsupported command"))})
	}
}

func (h *Handler) saveDraft(command SaveDraft, emit func(Event)) {
	key := draftKey{ChatID: command.ChatID, TopicID: command.TopicID}
	h.draftMutex.Lock()
	if h.draftPending == nil {
		h.draftPending = make(map[draftKey]draftSaveWork)
	}
	if h.draftRunning == nil {
		h.draftRunning = make(map[draftKey]bool)
	}
...
// resolveRuntime resolves runtime dependencies. Authorization prompts are not
// handled here: they arrive on the prompt subscription owned by AppModel, so
// resolution only waits for its own result.
func (h *Handler) resolveRuntime() (config.Runtime, error) {
...
// Cmd turns one reducer effect into a Bubble Tea command. Bubble Tea owns the
// scheduling: the command runs off the update goroutine and returns exactly one
// message. Effects that legitimately yield more than one result (a downloaded
// thumbnail also renders one) return a tea.Batch so each result stays a single
// command result.
func (h *Handler) Cmd(effect Effect) tea.Cmd {
	if effect == nil {
		return nil
	}
	return func() tea.Msg {
		var cmds []tea.Cmd
		h.dispatch(effect, func(event Event) {
...
// runCmds executes collected command results and returns a batch message when
// there is more than one. It mirrors tea.Batch's compact behavior.
...
// Updates returns the Telegram update stream for the single subscription.
func (h *Handler) Updates() <-chan Event {
	return h.updates
}

// PromptStream returns the authorization prompt stream for its subscription,
// or nil when the handler has no prompt broker.
func (h *Handler) PromptStream() <-chan auth.Prompt {
	if h.prompts == nil {
		return nil
	}
	return h.prompts.Prompts()
}

// waitForUpdate subscribes to one Telegram update. AppModel re-arms it after
// each update, so exactly one reader is outstanding.
func waitForUpdate(updates <-chan Event) tea.Cmd {
	if updates == nil {
		return nil
	}
	return func() tea.Msg {
		event, ok := <-updates
		if !ok {
			return telegramStreamClosed{}
		}
		return event
	}
}

// waitForPrompt subscribes to one authorization prompt. AppModel re-arms it
// after each prompt.
func waitForPrompt(prompts <-chan auth.Prompt) tea.Cmd {
	if prompts == nil {
		return nil
	}
	return func() tea.Msg {
		prompt, ok := <-prompts
		if !ok {
			return promptStreamClosed{}
		}
		return PromptRequested{Prompt: prompt}
	}
}

// telegramStreamClosed reports that the Telegram update stream ended. The
// terminal failure itself is delivered as a startup/operation failure message
// before the stream closes, so this only stops re-arming.
type telegramStreamClosed struct{}

// promptStreamClosed reports that the authorization prompt stream ended.
type promptStreamClosed struct{}

// startCmd resolves runtime dependencies and starts the Telegram client pump.
// It returns a startup failure when startup cannot proceed, otherwise nil.
func (h *Handler) startCmd() tea.Cmd {
	return func() tea.Msg { return h.start() }
}

func (h *Handler) start() tea.Msg {
	h.mutex.Lock()
	if h.client != nil {
		h.mutex.Unlock()
		return nil
	...
	if h.resolver == nil || h.factory == nil {
		h.mutex.Unlock()
		return StartupFailed{Error: safeError("start Telegram", errors.New("startup dependencies unavailable"))}
	}
	runtime, err := h.resolveRuntime()
	if err != nil {
		h.mutex.Unlock()
		return StartupFailed{Error: safeError("resolve runtime", err)}
	}
	client, err := h.factory(runtime, h.prompts)
	if err != nil {
		h.mutex.Unlock()
		return StartupFailed{Error: safeError("create Telegram client", err)}
	}
	if client == nil {
		h.mutex.Unlock()
		return StartupFailed{Error: safeError("create Telegram client", errors.New("client factory returned no client"))}
	}
	h.client = client
	rawUpdates := make(chan telegram.Update, 64)
	done := make(chan error, 1)
	go func() {
		h.startOnce.Do(func() { close(h.startBegan) })
		done <- client.Start(h.workCtx, rawUpdates)
		close(rawUpdates)
	}()
	h.mutex.Unlock()

	// The pump goroutine forwards raw updates as subscription messages, then
	// reports the terminal failure (if any) and closes the stream so the single
	// outstanding waitForUpdate observes the failure before stream closure.
	go func() {
		ready := false
		for update := range rawUpdates {
			...
			h.pushUpdate(TelegramEvent{Value: update, ReceivedAt: h.now()})
		}
		startErr := <-done
		if h.workCtx.Err() == nil || (startErr != nil && !errors.Is(startErr, context.Canceled) && !errors.Is(startErr, context.DeadlineExceeded)) {
			...
			if ready {
				h.pushUpdate(OperationFailed{Error: failure})
			} else {
				h.pushUpdate(StartupFailed{Error: failure})
			}
		}
		close(h.updates)
	}()
...
// pushUpdate delivers one stream message to the single update subscription,
// stopping when the work context ends.
func (h *Handler) pushUpdate(event Event) {
	select {
	case h.updates <- event:
	case <-h.workCtx.Done():
	}
}
