...
func messageSearchFastPathState(t *testing.T) State {
	...
	state, _ = updateState(state, MessageSearchValueChanged{ChatID: 9, Value: "needle"})
...
func chatSearchFastPathState(t *testing.T) State {
	t.Helper()
	state, commands := updateState(chatSearchBaseState(), ActionReceived{Action: OpenChatSearch})
...
func chatSettingsFastPathState(t *testing.T) State {
	...
	var commands []Effect
	state, commands = updateState(state, ActionReceived{Action: OpenChatSettings})
	if state.ChatSettings == nil || len(commands) != 1 {
		t.Fatalf("open chat settings = %#v %#v", state.ChatSettings, commands)
	}
	state, _ = updateState(state, ChatSettingsLoaded{
	...
	state, _ = updateState(state, ChatSettingsValueChanged{
		ChatID:   9,
...
func TestPromptValueChangedFastPathClonesOnlyPrompt(t *testing.T) {
	...
	got, commands := updateState(input, PromptValueChanged{PromptID: 41, Value: value})
...
func TestPhotoPathValueChangedFastPathClonesOnlyPhotoSend(t *testing.T) {
	...
	got, commands := updateState(input, PhotoPathValueChanged{ChatID: 9, Value: " /tmp/界🙂\nnext\rline "})
...
func TestSearchAndSettingsValueFastPathsOwnOnlyTheirOverlay(t *testing.T) {
	t.Run("message search", func(t *testing.T) {
		...
		pre := cloneReducerState(input)
		got, commands := updateState(input, MessageSearchValueChanged{ChatID: 9, Value: "replacement"})
	...
	t.Run("chat search", func(t *testing.T) {
		...
		pre := cloneReducerState(input)
		got, commands := updateState(input, ChatSearchValueChanged{Value: "beta"})
	...
	t.Run("chat settings", func(t *testing.T) {
		...
		pre := cloneReducerState(input)
		got, commands := updateState(input, ChatSettingsValueChanged{ChatID: 9, Field: ChatSettingTitle, EditorID: 41, Value: "new"})
...
func TestComposerValueFastPathOwnsDraftSyncSnapshots(t *testing.T) {
	...
	got, commands := updateState(input, ComposerValueChanged{ChatID: 9, Value: "new"})
