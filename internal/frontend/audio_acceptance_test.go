...
func TestAudioRenderingCaptionMetadataOmissionFallbackAndIdentity(t *testing.T) {
	...
	pendingRows, _ := buildMessageRows(ui.RenderedMessageGroup{MessageGroup: ui.MessageGroup{Messages: []domain.Message{pending}}}, 100, time.UTC, messageSelection{}, nil)
	...
	failedRows, _ := buildMessageRows(ui.RenderedMessageGroup{MessageGroup: ui.MessageGroup{Messages: []domain.Message{failed}}}, 100, time.UTC, messageSelection{}, nil)
...
func TestAudioActionLabelsAndPhotoVideoNonRegression(t *testing.T) {
	...
	for _, test := range []struct {
		...
		menu := &app.MessageActionMenu{ChatID: 9, MessageID: 77, MediaFile: file, MediaKind: test.kind}
		options := selectorOptionsFromRows(messageActionRows(menu))
		if len(options) != 1 || options[0].ID != test.id || options[0].Label != test.label || options[0].Value.Action != app.ViewMessageMedia {
