...
func TestInviteLinksGatedByChatCapability(t *testing.T) {
	state, commands := updateState(inviteState(false), ActionReceived{Action: OpenInviteLinks, ChatID: 9})
	...
	state, commands = updateState(inviteState(true), ActionReceived{Action: OpenInviteLinks, ChatID: 9})
...
func TestInviteLinksPagesDeduplicateAndIgnoreStaleResults(t *testing.T) {
	state, _ := updateState(inviteState(true), ActionReceived{Action: OpenInviteLinks, ChatID: 9})
	request := state.InviteLinks.RequestID
	state, _ = updateState(state, InviteLinksLoaded{RequestID: request, ChatID: 9, Page: telegram.InviteLinkPage{
	...
	state, _ = updateState(state, InviteLinksLoaded{RequestID: request, ChatID: 9, Page: telegram.InviteLinkPage{Links: []telegram.InviteLink{{URL: "stale"}}}})
...
func TestInviteLinksNavigationCreateCopyRevokeAndReload(t *testing.T) {
	state, _ := updateState(inviteState(true), ActionReceived{Action: OpenInviteLinks, ChatID: 9})
	request := state.InviteLinks.RequestID
	state, _ = updateState(state, InviteLinksLoaded{RequestID: request, ChatID: 9, Page: telegram.InviteLinkPage{Primary: &telegram.InviteLink{URL: "primary"}, Links: []telegram.InviteLink{{URL: "one"}}, Done: true}})
	state, _ = updateState(state, ActionReceived{Action: OpenInviteLinkDetail, ChatID: 9, InviteURL: "one"})
	if state.InviteLinks.DetailURL != "one" {
		t.Fatal("link detail did not open")
	}
	state, _ = updateState(state, ActionReceived{Action: CopyInviteLink, ChatID: 9, InviteURL: "one"})
	copyRequest := state.InviteLinks.RequestID
	state, _ = updateState(state, InviteLinkCopied{RequestID: copyRequest, ChatID: 9, URL: "one"})
	...
	state, _ = updateState(state, ActionReceived{Action: RevokeInviteLink, ChatID: 9, InviteURL: "one"})
	if !state.InviteLinks.Confirming {
		t.Fatal("revoke did not require confirmation")
	}
	state, _ = updateState(state, ActionReceived{Action: CancelRevokeInviteLink, ChatID: 9})
	if state.InviteLinks.Confirming {
		t.Fatal("revoke cancellation failed")
	}
	state, _ = updateState(state, ActionReceived{Action: RevokeInviteLink, ChatID: 9, InviteURL: "one"})
	state, commands := updateState(state, ActionReceived{Action: ConfirmRevokeInviteLink, ChatID: 9, InviteURL: "one"})
	...
	state, commands = updateState(state, InviteLinkRevoked{RequestID: revokeRequest, ChatID: 9, URL: "one"})
...
func TestInviteLinksFailuresAreSanitized(t *testing.T) {
	state, _ := updateState(inviteState(true), ActionReceived{Action: OpenInviteLinks, ChatID: 9})
	request := state.InviteLinks.RequestID
	state, _ = updateState(state, InviteLinksLoadFailed{RequestID: request, ChatID: 9})
	...
	state, _ = updateState(state, ActionReceived{Action: Retry, ChatID: 9})
	request = state.InviteLinks.RequestID
	state, _ = updateState(state, InviteLinkCreateFailed{RequestID: request, ChatID: 9})
