...
func TestComputeLayoutBreakpoints(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "wide boundary", width: 120, height: 24, want: LayoutWide},
		{name: "below wide width", width: 119, height: 24, want: LayoutNormal},
		{name: "normal boundary", width: 80, height: 20, want: LayoutNormal},
		{name: "below normal width", width: 79, height: 20, want: LayoutNarrow},
		{name: "narrow boundary", width: 60, height: 18, want: LayoutNarrow},
		{name: "below minimum width", width: 59, height: 30, want: LayoutTooSmall},
		{name: "below minimum height", width: 120, height: 17, want: LayoutTooSmall},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			got := ComputeLayout(test.width, test.height, false, FocusChats)
...
func TestComputeLayoutDegenerateTerminalHasNoRectangles(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "zero width", width: 0, height: 24, focus: FocusChats},
		{name: "negative width", width: -1, height: 24, details: true, focus: FocusDetails},
		{name: "zero height", width: 120, height: 0, focus: FocusConversation},
		{name: "negative height", width: 120, height: -1, details: true, focus: FocusDetails},
		{name: "both zero", width: 0, height: 0, details: true, focus: FocusDetails},
		{name: "both negative", width: -1, height: -1, focus: FocusChats},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if got.Mode != LayoutTooSmall {
				t.Errorf("ComputeLayout(%d, %d).Mode = %v, want %v", test.width, test.height, got.Mode, LayoutTooSmall)
			}
...
func TestComputeLayoutPanes(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "wide without details", width: 120, height: 24, focus: FocusChats,
			want: ViewLayout{
				Mode:         LayoutWide,
		...
		{
			name: "wide with details", width: 140, height: 25, details: true, focus: FocusDetails,
			want: ViewLayout{
				Mode:         LayoutWide,
		...
		{
			name: "normal without details", width: 100, height: 22, focus: FocusConversation,
			want: ViewLayout{
				Mode:         LayoutNormal,
		...
		{
			name: "normal with details", width: 100, height: 22, details: true, focus: FocusChats,
			want: ViewLayout{
				Mode:    LayoutNormal,
				Status:  image.Rect(0, 0, 100, 1),
		...
		{
			name: "narrow chats focus", width: 79, height: 20, focus: FocusChats,
			want: ViewLayout{
				Mode:   LayoutNarrow,
		...
		{
			name: "narrow conversation focus", width: 79, height: 20, focus: FocusConversation,
			want: ViewLayout{
				Mode:         LayoutNarrow,
				Status:       image.Rect(0, 0, 79, 1),
		...
		{
			name: "narrow details override focus", width: 60, height: 18, details: true, focus: FocusChats,
			want: ViewLayout{
				Mode:    LayoutNarrow,
		...
		{
			name: "too small has no body panes", width: 120, height: 17, details: true, focus: FocusDetails,
			want: ViewLayout{
				Mode:   LayoutTooSmall,
				Status: image.Rect(0, 0, 120, 1),
			},
		...
		{
			name: "too small without details has no body panes", width: 59, height: 30, focus: FocusChats,
			want: ViewLayout{
				Mode:   LayoutTooSmall,
...
func TestComputeLayoutPanesAreWithinTerminalAndDoNotOverlap(t *testing.T) {
	...
	focuses := []Focus{FocusChats, FocusConversation, FocusComposer, FocusDetails, FocusModal, FocusAuth}
...
func TestHitMapUsesReversePriority(t *testing.T) {
	...
	hits := HitMap{
		{Rect: image.Rect(0, 0, 10, 10), Click: ActionReceived{Action: SelectNext}},
		{Rect: image.Rect(2, 2, 8, 8), Click: ActionReceived{Action: Activate}},
	}

	got, ok := hits.ActionAt(3, 3)
	if !ok || got.Action != Activate {
...
func TestHitMapNoActionFallsThroughToLowerHit(t *testing.T) {
	...
	hits := HitMap{
		{Rect: image.Rect(0, 0, 10, 10), Click: ActionReceived{Action: SelectPrevious}},
		{Rect: image.Rect(0, 0, 10, 10), Click: ActionReceived{Action: NoAction, AvatarKey: "ignored"}},
	}

	got, ok := hits.ActionAt(5, 5)
	if !ok || got.Action != SelectPrevious {
...
func TestHitMapHonorsHalfOpenRectangles(t *testing.T) {
	...
	hits := HitMap{{Rect: image.Rect(2, 3, 5, 7), Click: ActionReceived{Action: Activate}}}
	...
	if got, ok := hits.ActionAt(4, 6); !ok || got.Action != Activate {
...
func TestHitMapMapsWheelsIndependently(t *testing.T) {
	...
	wantUp := ActionReceived{Action: PageUp, ChatID: domain.ChatID(17), At: time.Unix(50, 0)}
	wantDown := ActionReceived{Action: PageDown, MessageID: domain.MessageID(23), TargetFocus: FocusDetails}
	hits := HitMap{{
...
func TestHitMapWheelPriorityFallbackAndOutside(t *testing.T) {
	...
	hits := HitMap{
		...
		},
		{
			...
			WheelUp:   ActionReceived{Action: NoAction},
			WheelDown: ActionReceived{Action: SelectNext},
		},
	}

	if got, ok := hits.WheelAt(5, 5, true); !ok || got.Action != PageUp {
		t.Fatalf("WheelAt(up) = (%#v, %t), want lower PageUp", got, ok)
	}
	if got, ok := hits.WheelAt(5, 5, false); !ok || got.Action != SelectNext {
