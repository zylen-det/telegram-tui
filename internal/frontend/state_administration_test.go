...
func TestAdministrationPermissionToggleAndSave(t *testing.T) {
	...
	cmd := updateState(&state, ActionReceived{Action: OpenGroupPermissions})
	...
	updateState(&state, AdministrationLoaded{RequestID: id, ChatID: 1, Snapshot: telegram.AdministrationSnapshot{Kind: domain.ChatSupergroup, CanRestrictMembers: true}})
	updateState(&state, ActionReceived{Action: ToggleAdministrationItem, AdminIndex: 0})
	if !state.Administration.EditedPermissions.CanSendBasicMessages {
		t.Fatal("toggle failed")
	}
	cmd = updateState(&state, ActionReceived{Action: SaveAdministration})
...
func TestAdministrationStaleAndSanitizedFailure(t *testing.T) {
	...
	id := state.Administration.RequestID
	updateState(&state, AdministrationLoadFailed{RequestID: id + 1, ChatID: 1})
	if state.Administration.Error != nil {
		t.Fatal("stale failure applied")
	}
	updateState(&state, AdministrationLoadFailed{RequestID: id, ChatID: 1})
