...
func TestSelectPublishesHistoryErrorWithoutCause(t *testing.T) {
	...
	state.History[9] = HistoryState{Error: &domain.AppError{Kind: domain.ErrorNetwork, Message: "offline", Cause: errors.New("raw")}}
...
func TestLiveHistoryViewModelPublishesLoadingAndDone(t *testing.T) {
	...
	state.History[9] = HistoryState{Loading: true, Done: true}
	model := Select(state, time.UTC)
...
func TestPinnedMarkerMenuProjectionSurvivesSelect(t *testing.T) {
	...
	state.MessageMenu = &MessageActionMenu{ChatID: 9, MessageID: 2, Capabilities: domain.MessageCapabilities{Copy: true, Pin: true}, Pinned: true}
	model := Select(state, time.UTC)
...
func TestMessageActionMenuProjectionExposesOnlyCopyCapability(t *testing.T) {
	...
	state.SelectedMessageChat, state.SelectedMessage = 9, 2
	state.MessageMenu = &MessageActionMenu{ChatID: 9, MessageID: 2, Capabilities: domain.MessageCapabilities{Copy: true}}
	model := Select(state, time.UTC)
...
func TestSelectBuildsRenderModelContinuation(t *testing.T) {
	...
	state.History[9] = HistoryState{ViewOffset: 3, FollowSelection: true}
	state.ChatsLoading = true
	state.ChatsLoaded = true
	state.ChatsError = &domain.AppError{Kind: domain.ErrorNetwork, Message: "refreshing cached chats"}
	state.Avatars["mina-avatar:chat-list"] = AvatarState{Cells: testAvatar('M')}
	state.Avatars["weekend-avatar:chat-list"] = AvatarState{Cells: testAvatar('W')}
	state.Avatars["iris-avatar:message-group"] = AvatarState{Cells: testAvatar('I')}
	...
	wantLayout := ComputeLayout(140, 30, true, FocusDetails)
	if got.Layout != wantLayout {
		t.Fatalf("Layout = %#v, want freshly computed %#v", got.Layout, wantLayout)
	}
	if got.Focus != FocusDetails || got.Connection != domain.ConnectionReconnecting || !got.DetailsOpen {
...
func TestInitialStateInitializesAvatars(t *testing.T) {
	...
	state.Avatars["key"] = AvatarState{Loading: true}
}
...
func TestSelectPhotoSend(t *testing.T) {
	...
	if model.PhotoSend.PreviousFocus != FocusConversation {
		t.Errorf("PreviousFocus = %v, want %v", model.PhotoSend.PreviousFocus, FocusConversation)
	}
...
func TestSelectPlainChatGoldenUnchanged(t *testing.T) {
	...
	state.History[9] = HistoryState{ViewOffset: 3, Done: true}
...
func TestSelectTopicDraftAndHistory(t *testing.T) {
	...
	state.TopicDrafts[TopicKey{ChatID: 9, TopicID: 1}] = "topic draft"
	state.TopicHistory[TopicKey{ChatID: 9, TopicID: 1}] = HistoryState{ViewOffset: 4, FollowSelection: true, Done: true}
...
func TestSelectTopicDraftZeroHistoryWhenAbsent(t *testing.T) {
	...
	state.ForumTopics[9] = map[domain.TopicID]domain.ForumTopic{1: {ID: 1, ChatID: 9, Name: "General"}}
	state.SelectedTopics[9] = 1
	state.History[9] = HistoryState{ViewOffset: 99, Done: true}
...
func TestSelectShowAllRoutesDraftAndChatHistory(t *testing.T) {
	...
	state.Drafts[9] = "chat-level draft (must be ignored)"
	state.TopicDrafts[TopicKey{ChatID: 9, TopicID: 0}] = "ALL draft"
	state.History[9] = HistoryState{ViewOffset: 5, Done: true}
	state.TopicHistory[TopicKey{ChatID: 9, TopicID: 1}] = HistoryState{ViewOffset: 42}
...
func TestSelectShowAllEditTargetOverridesDraft(t *testing.T) {
	...
	state.ShowAll[9] = true
	state.TopicDrafts[TopicKey{ChatID: 9, TopicID: 0}] = "ALL draft"
	state.EditTarget = &EditTarget{ChatID: 9, Buffer: "editing buffer"}
