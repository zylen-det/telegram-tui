...
func openMembers(state State) (State, []Effect) {
	...
	return state, []Effect{LoadMembers{
		RequestID: requestID,
		ChatID:    chatID,
...
func openUserInfo(state State, chatID domain.ChatID, userID domain.UserID, previousFocus Focus) (State, []Effect) {
	if userID == 0 {
		return state, nil
	}
	requestID := allocateRequestID(&state)
	...
	state.Focus = FocusMembers
	return state, []Effect{LoadUserInfo{RequestID: requestID, ChatID: chatID, UserID: userID}}
}

func reduceUserInfoLoaded(state State, event UserInfoLoaded) (State, []Effect) {
...
func reduceUserInfoFailed(state State, event UserInfoLoadFailed) (State, []Effect) {
	members := state.Members
	if members == nil || !members.Single || members.RequestID != event.RequestID || members.ChatID != event.ChatID {
		return state, nil
	}
...
func reduceMembersAction(state State, event ActionReceived) (State, []Effect) {
	members := state.Members
	if members == nil {
		return state, nil
	}
...
func reduceMemberDetailActionRouter(state State, event ActionReceived) (State, []Effect) {
	members := state.Members
	switch event.Action {
	case Close:
...
func maybePaginateMembers(state State) (State, []Effect) {
	members := state.Members
...
func requestNextMembersPage(state State) (State, []Effect) {
	members := state.Members
...
func reduceMembersLoaded(state State, event MembersLoaded) (State, []Effect) {
	members := state.Members
...
func reduceMembersLoadFailed(state State, event MembersLoadFailed) (State, []Effect) {
	members := state.Members
	if members == nil || members.RequestID != event.RequestID || members.ChatID != event.ChatID {
		return state, nil
	}
