...
func TestChatSearchOpenAndSubmitExactUsername(t *testing.T) {
	...
	opened, commands := Reduce(state, ActionReceived{Action: OpenChatSearch})
	...
	changed, _ := Reduce(opened, ChatSearchValueChanged{Value: "  @BotFather\n"})
	if got := string(changed.ChatSearch.Input); got != "  @BotFather" {
		t.Fatalf("input = %q", got)
	}
	submitted, commands := Reduce(changed, ActionReceived{Action: SubmitChatSearch})
	if len(commands) != 3 {
	...
	blank, _ := Reduce(opened, ChatSearchValueChanged{Value: " @ "})
	blank, blankCommands := Reduce(blank, ActionReceived{Action: SubmitChatSearch})
...
func TestChatSearchOpensOnlyFromChatsAndCloses(t *testing.T) {
	...
	unchanged, commands := Reduce(state, ActionReceived{Action: OpenChatSearch})
	if len(commands) != 0 || unchanged.ChatSearch != nil {
		t.Fatalf("conversation open = %#v %#v", unchanged.ChatSearch, commands)
	}

	opened, _ := Reduce(chatSearchBaseState(), ActionReceived{Action: OpenChatSearch})
	closed, commands := Reduce(opened, ActionReceived{Action: Close})
...
func TestChatSearchResultsStaleGuardAndEditQuery(t *testing.T) {
	state := chatSearchBaseState()
	opened, _ := Reduce(state, ActionReceived{Action: OpenChatSearch})
	opened, _ = Reduce(opened, ChatSearchValueChanged{Value: "@botfather"})
	submitted, _ := Reduce(opened, ActionReceived{Action: SubmitChatSearch})
	...
	stale, commands := Reduce(submitted, PublicChatsSearched{RequestID: submitted.ChatSearch.RequestID + 99, Chats: []domain.Chat{found}})
	...
	loaded, _ := Reduce(submitted, PublicChatsSearched{RequestID: requestID, Chats: []domain.Chat{found}})
	...
	edited, commands := Reduce(loaded, ActionReceived{Action: OpenChatSearch})
	...
	staleAfterEdit, _ := Reduce(edited, PublicChatSearchFailed{RequestID: 10, Error: domain.AppError{Message: "raw"}})
...
func TestChatSearchActivationAddsSelectsAndOpensChat(t *testing.T) {
	...
	loaded, _ := Reduce(submitted, PublicChatsSearched{RequestID: requestID, Chats: []domain.Chat{found}})
	loaded, _ = Reduce(loaded, AllMessagesSearched{RequestID: requestID, Messages: []domain.Message{}})
	activated, commands := Reduce(loaded, ActionReceived{Action: Activate})
...
func TestChatSearchMouseSelectionRequiresReturnedIdentity(t *testing.T) {
	...
	submitted, _ := Reduce(opened, ActionReceived{Action: SubmitChatSearch})
	requestID := submitted.ChatSearch.RequestID
	loaded, _ := Reduce(submitted, PublicChatsSearched{RequestID: requestID, Chats: []domain.Chat{{ID: 99, Title: "BotFather"}}})
	loaded, _ = Reduce(loaded, AllMessagesSearched{RequestID: requestID, Messages: []domain.Message{}})
	unchanged, commands := Reduce(loaded, ActionReceived{Action: SelectChat, ChatID: 100})
	if len(commands) != 0 || unchanged.ChatSearch == nil {
		t.Fatalf("wrong result = %#v %#v", unchanged.ChatSearch, commands)
	}
	selected, commands := Reduce(loaded, ActionReceived{Action: SelectChat, ChatID: 99})
...
func TestHandlerPublicChatSearchUsesExactUsernameAndSafeFailure(t *testing.T) {
	client := &handlerClient{publicChat: domain.Chat{ID: 99, Title: "BotFather", Username: "BotFather"}}
	handler := newTestHandler(t, client, &handlerAvatarRenderer{})
	events := collectHandlerEvents(handler, SearchPublicChatCommand{RequestID: 8, Username: "BotFather"})
	if len(events) != 1 {
		t.Fatalf("events = %#v", events)
	}
	searched, ok := events[0].(PublicChatSearched)
	if !ok || searched.RequestID != 8 || searched.Chat.ID != 99 || client.publicUsername != "BotFather" {
		t.Fatalf("event/client = %#v username=%q", events[0], client.publicUsername)
	}

	client.err = errors.New("private transport detail")
	events = collectHandlerEvents(handler, SearchPublicChatCommand{RequestID: 9, Username: "secret_name"})
	failed, ok := events[0].(PublicChatSearchFailed)
	if !ok || failed.RequestID != 9 || failed.Error.Message != "Could not find public chat" || failed.Error.Cause != nil {
...
func TestChatSearchValueChangedFiltersLocalChats(t *testing.T) {
	...
	opened, _ := Reduce(state, ActionReceived{Action: OpenChatSearch})
	changed, _ := Reduce(opened, ChatSearchValueChanged{Value: "tele"})
...
func TestChatSearchValueChangedClearsWhenBlank(t *testing.T) {
	...
	changed, _ := Reduce(opened, ChatSearchValueChanged{Value: "tele"})
	// Now clear.
	cleared, _ := Reduce(changed, ChatSearchValueChanged{Value: ""})
...
func TestChatSearchPublicChatsSearched(t *testing.T) {
	state := chatSearchBaseState()
	opened, _ := Reduce(state, ActionReceived{Action: OpenChatSearch})
	opened, _ = Reduce(opened, ChatSearchValueChanged{Value: "test"})
	submitted, _ := Reduce(opened, ActionReceived{Action: SubmitChatSearch})
	publicChats := []domain.Chat{
		{ID: 100, Title: "Public Group", Username: "publicgroup", Order: 20},
	}
	requestID := submitted.ChatSearch.RequestID
	loaded, _ := Reduce(submitted, PublicChatsSearched{RequestID: requestID, Chats: publicChats})
...
func TestChatSearchPublicChatsSearchFailed(t *testing.T) {
	...
	submitted, _ := Reduce(opened, ActionReceived{Action: SubmitChatSearch})
	requestID := submitted.ChatSearch.RequestID
	failed, _ := Reduce(submitted, PublicChatsSearchFailed{RequestID: requestID, Error: domain.AppError{Message: "network error"}})
...
func TestChatSearchAllMessagesSearched(t *testing.T) {
	state := chatSearchBaseState()
	opened, _ := Reduce(state, ActionReceived{Action: OpenChatSearch})
	opened, _ = Reduce(opened, ChatSearchValueChanged{Value: "needle"})
	submitted, _ := Reduce(opened, ActionReceived{Action: SubmitChatSearch})
	messages := []domain.Message{
		{ID: 1, ChatID: 7, Kind: domain.MessageText, Text: "This has a needle", SentAt: time.Now()},
	}
	requestID := submitted.ChatSearch.RequestID
	loaded, _ := Reduce(submitted, AllMessagesSearched{RequestID: requestID, Messages: messages})
...
func TestChatSearchAllMessagesSearchFailed(t *testing.T) {
	...
	submitted, _ := Reduce(opened, ActionReceived{Action: SubmitChatSearch})
	requestID := submitted.ChatSearch.RequestID
	failed, _ := Reduce(submitted, AllMessagesSearchFailed{RequestID: requestID, Error: domain.AppError{Message: "search error"}})
...
func TestChatSearchLiveTypingEmitsBothAndStaysInInput(t *testing.T) {
	...
	opened, _ := Reduce(state, ActionReceived{Action: OpenChatSearch})
	typed, commands := Reduce(opened, ChatSearchValueChanged{Value: "tele"})
...
func TestChatSearchRemoteArrivalPreservesInputFocus(t *testing.T) {
	state := chatSearchBaseState()
	opened, _ := Reduce(state, ActionReceived{Action: OpenChatSearch})
	typed, _ := Reduce(opened, ChatSearchValueChanged{Value: "tele"})
	requestID := typed.ChatSearch.RequestID
	loaded, _ := Reduce(typed, PublicChatsSearched{RequestID: requestID, Chats: []domain.Chat{{ID: 99, Title: "Pub"}}})
	...
	loaded, _ = Reduce(loaded, AllMessagesSearched{RequestID: requestID, Messages: []domain.Message{{ID: 5, ChatID: 7, Kind: domain.MessageText, Text: "hi"}}})
...
func TestChatSearchMessageActivationJumpsToContext(t *testing.T) {
	state := chatSearchBaseState()
	opened, _ := Reduce(state, ActionReceived{Action: OpenChatSearch})
	typed, _ := Reduce(opened, ChatSearchValueChanged{Value: "needle"})
	requestID := typed.ChatSearch.RequestID
	loaded, _ := Reduce(typed, PublicChatsSearched{RequestID: requestID, Chats: nil})
	msg := domain.Message{ID: 20, ChatID: 7, Kind: domain.MessageText, Text: "needle hit", SentAt: time.Now()}
	loaded, _ = Reduce(loaded, AllMessagesSearched{RequestID: requestID, Messages: []domain.Message{msg}})
	activated, commands := Reduce(loaded, ActionReceived{Action: Activate})
	if activated.ChatSearch != nil {
	...
	landed, _ := Reduce(activated, SearchMessageContextLoaded{RequestID: activated.MessageSearch.RequestID, ChatID: 7, MessageID: 20, Page: page})
...
func TestChatSearchMouseMessageSelectionJumps(t *testing.T) {
	...
	requestID := typed.ChatSearch.RequestID
	loaded, _ := Reduce(typed, PublicChatsSearched{RequestID: requestID, Chats: nil})
	loaded, _ = Reduce(loaded, AllMessagesSearched{RequestID: requestID, Messages: []domain.Message{{ID: 20, ChatID: 7, Kind: domain.MessageText, Text: "hit"}}})
	selected, commands := Reduce(loaded, ActionReceived{Action: SelectMessage, ChatID: 7, MessageID: 20})
