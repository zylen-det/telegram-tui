...
func openTopics(state State) (State, []Command) {
	...
	return state, []Command{LoadTopics{
		RequestID: requestID,
		ChatID:    chatID,
...
func reduceTopicsAction(state State, event ActionReceived) (State, []Command) {
	topics := state.Topics
	if topics == nil {
		return state, nil
	}
...
func activateAllTopics(state State, chatID domain.ChatID) (State, []Command) {
	...
	if _, exists := state.History[chatID]; !exists {
		...
		return state, []Command{LoadMessages{RequestID: requestID, ChatID: chatID, Cursor: telegram.MessageCursor{Limit: pageSize}}}
	}
	return state, nil
}

func requestNextTopicsPage(state State) (State, []Command) {
	topics := state.Topics
...
func activateTopic(state State, topic domain.ForumTopic) (State, []Command) {
...
func reduceTopicsLoaded(state State, event TopicsLoaded) (State, []Command) {
	topics := state.Topics
...
func reduceTopicsLoadFailed(state State, event TopicsLoadFailed) (State, []Command) {
	topics := state.Topics
	if topics == nil || topics.RequestID != event.RequestID || topics.ChatID != event.ChatID {
		return state, nil
	}
...
// the topic request.
func reduceTopicMessagesLoaded(state State, event MessagesLoaded) (State, []Command) {
...
func reduceForumTopicInfoChanged(state State, update telegram.ForumTopicInfoChanged) (State, []Command) {
	topic := update.Topic
...
func reduceForumTopicStateChanged(state State, update telegram.ForumTopicStateChanged) (State, []Command) {
