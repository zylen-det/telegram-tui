...
func TestBotCommandMenuLoadsWithoutTakingComposerFocus(t *testing.T) {
	...
	got, commands := Reduce(input, ComposerValueChanged{ChatID: 9, Value: "/"})
...
func TestBotCommandMenuFiltersPrefixBeforeSubstringAndCompletesGroupTarget(t *testing.T) {
	state, commands := Reduce(botCommandState(), ComposerValueChanged{ChatID: 9, Value: "/"})
	...
	state, _ = Reduce(state, BotCommandsLoaded{RequestID: request.RequestID, ChatID: 9, Commands: catalog})
	state, commands = Reduce(state, ComposerValueChanged{ChatID: 9, Value: "/he"})
	...
	state, _ = Reduce(state, ActionReceived{Action: CommandMenuNext})
	state, commands = Reduce(state, ActionReceived{Action: CommandMenuActivate})
	if len(commands) != 1 {
...
func TestBotCommandMenuDismissesWithoutChangingDraftAndClosesAfterWhitespace(t *testing.T) {
	...
	state, _ = Reduce(state, ComposerValueChanged{ChatID: 9, Value: "/st"})
	state, _ = Reduce(state, ActionReceived{Action: CommandMenuDismiss})
	if state.CommandMenu != nil || state.Drafts[9] != "/st" || state.Focus != FocusComposer {
		t.Fatalf("dismiss = %#v", state)
	}
	state, _ = Reduce(state, ComposerValueChanged{ChatID: 9, Value: "/start arg"})
...
func TestBotCommandMenuRejectsStaleResultsAndMouseIdentity(t *testing.T) {
	state, commands := Reduce(botCommandState(), ComposerValueChanged{ChatID: 9, Value: "/"})
	request := commands[0].(LoadBotCommands)
	stale, _ := Reduce(state, BotCommandsLoaded{RequestID: request.RequestID + 1, ChatID: 9, Commands: []domain.BotCommand{{Name: "bad"}}})
	if !reflect.DeepEqual(stale, state) {
		t.Fatal("stale command result changed state")
	}
	state, _ = Reduce(state, BotCommandsLoaded{RequestID: request.RequestID, ChatID: 9, Commands: []domain.BotCommand{{Name: "start"}, {Name: "help"}}})
	unchanged, _ := Reduce(state, ActionReceived{Action: CommandMenuActivate, ChatID: 99, CommandIndex: 1})
	if !reflect.DeepEqual(unchanged, state) {
		t.Fatal("stale mouse identity changed state")
	}
	state, _ = Reduce(state, ActionReceived{Action: CommandMenuActivate, ChatID: 9, CommandIndex: 1})
...
func TestBotCommandMenuFailureIsInlineAndEditInputNeverOpensIt(t *testing.T) {
	state, commands := Reduce(botCommandState(), ComposerValueChanged{ChatID: 9, Value: "/"})
	request := commands[0].(LoadBotCommands)
	state, _ = Reduce(state, BotCommandsLoadFailed{RequestID: request.RequestID, ChatID: 9})
	...
	got, editCommands := Reduce(editing, ComposerValueChanged{ChatID: 9, EditMessageID: 7, Value: "/start"})
...
func TestBotCommandMenuSelectionScrollsWithinEightRows(t *testing.T) {
	...
	state, _ = Reduce(state, ComposerValueChanged{ChatID: 9, Value: "/"})
	for range 9 {
		state, _ = Reduce(state, ActionReceived{Action: CommandMenuNext})
	}
