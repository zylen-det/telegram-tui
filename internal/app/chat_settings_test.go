...
func TestActivateSelectedChatSettingsRowOpensSettings(t *testing.T) {
	...
	state, commands := Reduce(state, ActionReceived{Action: Activate})
...
func TestChatSettingsLoadEditSaveAndStaleResults(t *testing.T) {
	state, commands := Reduce(chatSettingsBase(), ActionReceived{Action: OpenChatSettings})
	...
	state, _ = Reduce(state, ChatSettingsLoaded{RequestID: request, ChatID: 9, Snapshot: telegram.ChatSettings{Kind: domain.ChatSupergroup, Title: "Old", Description: "About", SlowModeDelay: 30, CanChangeInfo: true, CanRestrictMembers: true}})
	...
	state, _ = Reduce(state, ChatSettingsValueChanged{ChatID: 9, Field: ChatSettingTitle, EditorID: editor, Value: "New"})
	state, commands = Reduce(state, ActionReceived{Action: SaveChatSetting, ChatID: 9})
	...
	state, _ = Reduce(state, ChatSettingSaved{RequestID: saveRequest, ChatID: 9, Field: ChatSettingDescription})
	...
	state, _ = Reduce(state, ChatSettingSaved{RequestID: saveRequest, ChatID: 9, Field: ChatSettingTitle})
...
func TestChatSettingsValidationAndSlowModeChoices(t *testing.T) {
	state, _ := Reduce(chatSettingsBase(), ActionReceived{Action: OpenChatSettings})
	request := state.ChatSettings.RequestID
	state, _ = Reduce(state, ChatSettingsLoaded{RequestID: request, ChatID: 9, Snapshot: telegram.ChatSettings{Kind: domain.ChatSupergroup, Title: "Old", CanChangeInfo: true, CanRestrictMembers: true, SlowModeDelay: 30}})
	state, _ = Reduce(state, ActionReceived{Action: EditChatSetting, ChatID: 9, SettingField: ChatSettingTitle})
	...
	state, _ = Reduce(state, ActionReceived{Action: Close, ChatID: 9})
	state, _ = Reduce(state, ActionReceived{Action: EditChatSetting, ChatID: 9, SettingField: ChatSettingSlowMode})
	...
	state, commands = Reduce(state, ActionReceived{Action: SelectChatSlowMode, ChatID: 9, SlowModeDelay: 300})
...
func TestChatSettingsTitleAndDescriptionKeepSeparateEditorState(t *testing.T) {
	state, _ := Reduce(chatSettingsBase(), ActionReceived{Action: OpenChatSettings})
	request := state.ChatSettings.RequestID
	state, _ = Reduce(state, ChatSettingsLoaded{RequestID: request, ChatID: 9, Snapshot: telegram.ChatSettings{Kind: domain.ChatSupergroup, Title: "Old title", Description: "Old description", CanChangeInfo: true}})
	state, _ = Reduce(state, ActionReceived{Action: EditChatSetting, ChatID: 9, SettingField: ChatSettingTitle})
	titleEditor := state.ChatSettings.TitleEditorID
	state, _ = Reduce(state, ChatSettingsValueChanged{ChatID: 9, Field: ChatSettingTitle, EditorID: titleEditor, Value: "Draft title"})
	state, _ = Reduce(state, ActionReceived{Action: Close, ChatID: 9})
	state, _ = Reduce(state, ActionReceived{Action: EditChatSetting, ChatID: 9, SettingField: ChatSettingDescription})
	...
	state, _ = Reduce(state, ChatSettingsValueChanged{ChatID: 9, Field: ChatSettingTitle, EditorID: titleEditor, Value: "stale"})
...
func TestChatSettingsFailureRetryAndCloseDiscardLateResults(t *testing.T) {
	state, _ := Reduce(chatSettingsBase(), ActionReceived{Action: OpenChatSettings})
	request := state.ChatSettings.RequestID
	state, _ = Reduce(state, ChatSettingsLoadFailed{RequestID: request, ChatID: 9})
	if state.ChatSettings.Error == nil || len(ChatSettingsMenuItems(state.ChatSettings)) != 1 {
		t.Fatal("load failure did not expose retry")
	}
	state, commands := Reduce(state, ActionReceived{Action: Retry, ChatID: 9})
	...
	state, _ = Reduce(state, ChatSettingsLoaded{RequestID: request, ChatID: 9, Snapshot: telegram.ChatSettings{Title: "late"}})
