...
func TestScalarReducerEventsDoNotCloneCollectionBranches(t *testing.T) {
	...
	got, commands := Reduce(input, TerminalFocusChanged{Focused: false})
...
func TestMessageEditedCopiesOnlyAffectedChatMessages(t *testing.T) {
	...
	got, commands := Reduce(input, TelegramEvent{Value: telegram.MessageEdited{ChatID: 9, MessageID: 1, EditedAt: editedAt}})
...
func TestChatUpsertCopiesDraftAndChatBranchesWithoutMessages(t *testing.T) {
	...
	got, commands := Reduce(input, TelegramEvent{Value: telegram.ChatUpserted{Chat: updated}})
...
func TestMessagesLoadedCopiesOnlyTargetAndLoadTrackingBranches(t *testing.T) {
	...
	got, commands := Reduce(input, MessagesLoaded{RequestID: 44, ChatID: 9, Page: page})
...
func TestMessageUpsertCopiesOnlyBranchesItUpdates(t *testing.T) {
	...
	got, commands := Reduce(input, TelegramEvent{Value: telegram.MessageUpserted{Message: message}})
...
func TestChatNavigationCopiesOnlyMutableSelectionBranches(t *testing.T) {
	...
	got, commands := Reduce(input, ActionReceived{Action: SelectNext})
...
func TestMessageSelectionNavigationCopiesHistoryOnly(t *testing.T) {
	...
	got, commands := Reduce(input, ActionReceived{Action: SelectPreviousMessage})
