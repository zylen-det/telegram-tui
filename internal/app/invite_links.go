...
func openInviteLinks(state State) (State, []Command) {
	...
	return state, []Command{LoadInviteLinksCommand{RequestID: requestID, ChatID: chatID, Cursor: telegram.InviteLinkCursor{Limit: pageSize}}}
}

func reduceInviteLinksAction(state State, event ActionReceived) (State, []Command) {
	...
	case CreateInviteLink:
		...
		links.Working = true
		return state, []Command{CreateInviteLinkCommand{RequestID: requestID, ChatID: links.ChatID}}
	case CopyInviteLink:
		...
		links.RequestID = requestID
		links.Working = true
		return state, []Command{CopyInviteLinkCommand{RequestID: requestID, ChatID: links.ChatID, URL: links.DetailURL}}
	...
	case ConfirmRevokeInviteLink:
		...
		links.Working = true
		links.Confirming = false
		return state, []Command{RevokeInviteLinkCommand{RequestID: requestID, ChatID: links.ChatID, URL: links.DetailURL}}
	}
	return state, nil
}
...
func maybePaginateInviteLinks(state State) (State, []Command) {
	links := state.InviteLinks
	items := InviteLinkMenuItems(links)
	...
	links.RequestID = requestID
	links.Loading = true
	return state, []Command{LoadInviteLinksCommand{RequestID: requestID, ChatID: links.ChatID, Cursor: links.NextCursor}}
}

func reloadInviteLinks(state State) (State, []Command) {
	links := state.InviteLinks
	if links == nil {
		return state, nil
	}
	...
	*links = InviteLinksState{RequestID: requestID, ChatID: links.ChatID, PreviousFocus: links.PreviousFocus, Loading: true}
	return state, []Command{LoadInviteLinksCommand{RequestID: requestID, ChatID: links.ChatID, Cursor: telegram.InviteLinkCursor{Limit: pageSize}}}
}

func reduceInviteLinksLoaded(state State, event InviteLinksLoaded) (State, []Command) {
	links := state.InviteLinks
...
func reduceInviteLinksLoadFailed(state State, event InviteLinksLoadFailed) (State, []Command) {
	links := state.InviteLinks
	if links == nil || links.RequestID != event.RequestID || links.ChatID != event.ChatID || !links.Loading {
		return state, nil
	}
...
func reduceInviteLinkCreated(state State, event InviteLinkCreated) (State, []Command) {
	links := state.InviteLinks
...
func reduceInviteLinkCreateFailed(state State, event InviteLinkCreateFailed) (State, []Command) {
	links := state.InviteLinks
...
func reduceInviteLinkRevoked(state State, event InviteLinkRevoked) (State, []Command) {
	links := state.InviteLinks
...
func reduceInviteLinkRevokeFailed(state State, event InviteLinkRevokeFailed) (State, []Command) {
	links := state.InviteLinks
	if links == nil || links.RequestID != event.RequestID || links.ChatID != event.ChatID || links.DetailURL != event.URL || !links.Working {
		return state, nil
	}
...
func reduceInviteLinkCopied(state State, event InviteLinkCopied) (State, []Command) {
	links := state.InviteLinks
	if links == nil || links.RequestID != event.RequestID || links.ChatID != event.ChatID || links.DetailURL != event.URL || !links.Working {
		return state, nil
	}
...
func reduceInviteLinkCopyFailed(state State, event InviteLinkCopyFailed) (State, []Command) {
	links := state.InviteLinks
	if links == nil || links.RequestID != event.RequestID || links.ChatID != event.ChatID || links.DetailURL != event.URL || !links.Working {
		return state, nil
	}
