...
func openChatActionMenu(state State) (State, []Command) {
...
func reduceChatActionMenu(state State, event ActionReceived) (State, []Command) {
...
func openActiveChatFromList(state State) (State, []Command) {
	chatID, ok := activeChatID(state)
	if !ok {
		return state, nil
	}
...
func executeChatAction(state State, action Action) (State, []Command) {
	menu := state.ChatActions
	if menu == nil {
		return state, nil
	}
	...
	menu.Working = true
	return state, []Command{ApplyChatActionCommand{RequestID: requestID, ChatID: menu.ChatID, Action: telegramAction}}
}
...
func reduceChatActionApplied(state State, event ChatActionApplied) (State, []Command) {
	menu := state.ChatActions
...
func reduceChatActionFailed(state State, event ChatActionFailed) (State, []Command) {
	menu := state.ChatActions
	if menu == nil || !menu.Working || menu.RequestID != event.RequestID || menu.ChatID != event.ChatID {
		return state, nil
	}
