...
// The frontend owns placement, while app.State owns the resulting navigation
...
func openStickerPicker(state State) (State, []Command) {
	...
	return state, []Command{LoadStickers{RequestID: requestID, ChatID: chatID, TopicID: topicID}}
}

func reduceStickersLoaded(state State, event StickersLoaded) (State, []Command) {
	...
	picker.Selected = 0
	picker.FirstRow = 0
	commands := make([]Command, 0, len(picker.Catalog))
...
func reduceStickersLoadFailed(state State, event StickersLoadFailed) (State, []Command) {
	picker := state.StickerPicker
	if picker == nil || picker.RequestID != event.RequestID || picker.ChatID != event.ChatID {
		return state, nil
	}
...
func reduceStickerThumbnailRendered(state State, event StickerThumbnailRendered) (State, []Command) {
	picker := state.StickerPicker
...
func reduceStickerThumbnailFailed(state State, event StickerThumbnailFailed) (State, []Command) {
	picker := state.StickerPicker
	requestID, requested := state.StickerThumbnailRequests[event.StickerFileID]
	if picker == nil || event.RequestID == 0 || picker.RequestID != event.PickerRequestID || !requested || requestID != event.RequestID {
		return state, nil
...
func reduceStickerPicker(state State, event ActionReceived) (State, []Command) {
	picker := state.StickerPicker
	switch event.Action {
	case Close:
...
func sendSelectedSticker(state State, event ActionReceived) (State, []Command) {
	picker := state.StickerPicker
