...
func TestLayoutBreakpoints(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(layoutName(tt.width, tt.height), func(t *testing.T) {
			state, commands := Reduce(InitialState(), Resized{Width: tt.width, Height: tt.height})
...
func TestDuplicateAvatarOpenedPreservesOriginalFocus(t *testing.T) {
	...
	opened, commands := Reduce(state, AvatarOpened{Title: "Mina", Path: "/tmp/mina.jpg"})
	if len(commands) != 0 {
		t.Fatalf("open commands = %#v, want none", commands)
	}
	assertModalFocusInvariant(t, opened)

	duplicate, commands := Reduce(opened, AvatarOpened{Title: "Other", Path: "/tmp/other.jpg"})
	...
	closed, commands := Reduce(duplicate, ActionReceived{Action: Close})
	if len(commands) != 0 {
...
func TestPromptSubmitClearsSensitiveInputAndRestoresFocus(t *testing.T) {
	...
	got, commands := Reduce(state, ActionReceived{Action: ComposerSubmit})
	...
	assertCommands(t, commands, []Command{SubmitPrompt{Response: auth.Response{
		PromptID: 44,
		Value:    "秘密",
	}}})
}
...
func TestTypedNilPointerEventsAreNoOps(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, commands := Reduce(state, tt.event)
