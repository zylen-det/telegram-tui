...
func TestMessageSelectionUsesChatScopedIdentityAndSurvivesMatchingUpdate(t *testing.T) {
	...
	selected, _ := Reduce(state, ActionReceived{Action: SelectMessage, ChatID: 9, MessageID: 2})
	...
	updated, _ := Reduce(selected, TelegramEvent{Value: telegram.MessageUpserted{Message: domain.Message{ID: 2, ChatID: 9}}})
	if updated.SelectedMessageChat != 9 || updated.SelectedMessage != 2 {
		t.Fatal("matching update cleared selection")
	}
	changed, _ := Reduce(updated, ActionReceived{Action: SelectChat, ChatID: 10})
...
func TestMessageActionMenuIsCapabilityAwareAndCopyCommandIsContentOpaqueToState(t *testing.T) {
	...
	copied, commands := Reduce(opened, ActionReceived{Action: CopyMessage})
...
func TestMessageSelectionClearsWhenSelectedMessageDisappears(t *testing.T) {
	...
	got, _ := Reduce(state, MessagesLoaded{ChatID: 9, Page: telegram.MessagePage{Messages: []domain.Message{{ID: 1, ChatID: 9}}, Done: true}})
...
func TestMessageKeyboardNavigationAndMenuIdentityRemainAtomic(t *testing.T) {
	...
	next, _ := Reduce(state, ActionReceived{Action: SelectNextMessage})
	...
	previous, _ := Reduce(next, ActionReceived{Action: SelectPreviousMessage})
	...
	_, commands := Reduce(opened, ActionReceived{Action: CopyMessage})
	if len(commands) != 1 {
...
func TestMessageNavigationViewportFollowsSelectionAndStopsAtEndpoints(t *testing.T) {
	...
	older, commands := Reduce(state, ActionReceived{Action: SelectPreviousMessage})
	...
	older, commands = Reduce(older, ActionReceived{Action: SelectPreviousMessage})
	...
	newer, commands := Reduce(older, ActionReceived{Action: SelectNextMessage})
	...
	paged, commands := Reduce(newer, ActionReceived{Action: PageDown})
	...
	atOldest, commands := Reduce(oldest, ActionReceived{Action: SelectPreviousMessage})
	...
	atNewest, commands := Reduce(state, ActionReceived{Action: SelectNextMessage})
...
func TestAuthoritativeMessageActionsLoadAndRejectStaleResults(t *testing.T) {
	...
	state.Messages[9] = []domain.Message{{ID: 2, ChatID: 9, Kind: domain.MessageText, Text: "private"}}
	state.SelectedMessageChat, state.SelectedMessage = 9, 2
	opened, commands := Reduce(state, ActionReceived{Action: OpenMessageActionMenu})
	...
	loaded, _ := Reduce(opened, MessagePropertiesLoaded{RequestID: request.RequestID, ChatID: 9, MessageID: 2, Capabilities: domain.MessageCapabilities{Copy: true, Reply: true, Edit: true}})
	...
	stale, _ := Reduce(loaded, MessagePropertiesLoaded{RequestID: request.RequestID + 1, ChatID: 9, MessageID: 2})
	if !stale.MessageMenu.Capabilities.Edit {
		t.Fatal("stale result replaced snapshot")
	}
	closed, _ := Reduce(opened, ActionReceived{Action: Close})
	closed, _ = Reduce(closed, MessagePropertiesLoaded{RequestID: request.RequestID, ChatID: 9, MessageID: 2, Capabilities: domain.MessageCapabilities{Edit: true}})
	if closed.MessageMenu != nil {
...
func TestAuthoritativeMessageActionsFailureIsSafe(t *testing.T) {
	...
	failed, _ := Reduce(opened, MessagePropertiesLoadFailed{RequestID: request.RequestID, ChatID: 9, MessageID: 2, Error: domain.AppError{Message: "raw secret-body 9 2"}})
...
func TestAuthoritativeMessageActionsCloseWhenTargetIsEvicted(t *testing.T) {
	...
	evicted, _ := Reduce(opened, TelegramEvent{Value: telegram.MessageUpserted{Message: domain.Message{ID: 999, ChatID: 9, Kind: domain.MessageText, SentAt: time.Unix(999, 0)}}})
...
func TestMessageMenuTracksOptimisticIdentityReplacement(t *testing.T) {
	...
	queued, _ := Reduce(state, TextQueued{LocalID: -1, Message: domain.Message{ID: -2, ChatID: 9, Kind: domain.MessageText}})
	...
	sent, _ := Reduce(queued, TelegramEvent{Value: telegram.MessageSendSucceeded{OldID: -2, Message: domain.Message{ID: 100, ChatID: 9, Kind: domain.MessageText}}})
...
func TestCopyMessageResultUsesOnlyConstantSafeToast(t *testing.T) {
	state := InitialState()
	succeeded, _ := Reduce(state, ClipboardWritten{})
	...
	failed, _ := Reduce(state, ClipboardWriteFailed{Error: failure})
...
func TestToastLifecycleReplacesAndRejectsStaleExpiry(t *testing.T) {
	state := InitialState()
	first, _ := Reduce(state, ClipboardWritten{})
	...
	second, _ := Reduce(first, OperationFailed{Error: domain.AppError{Kind: domain.ErrorNetwork, Message: "Try again"}})
	...
	stale, _ := Reduce(second, ToastExpired{Generation: firstGeneration})
	if stale.Toast == nil || stale.ToastGeneration != second.ToastGeneration {
		t.Fatal("stale expiry cleared replacement toast")
	}
	expired, _ := Reduce(second, ToastExpired{Generation: second.ToastGeneration})
...
func TestActionModalListNavigationAndActivation(t *testing.T) {
	...
	opened, _ = Reduce(opened, MessagePropertiesLoaded{RequestID: opened.MessageMenu.RequestID, ChatID: 9, MessageID: 2, Capabilities: domain.MessageCapabilities{Reply: true, Copy: true}})
	if opened.MessageMenu.Selected != 1 {
		t.Fatalf("authoritative actions did not preserve selected Copy action")
	}
	navigated, _ := Reduce(opened, ActionReceived{Action: SelectPrevious})
	if navigated.MessageMenu.Selected != 0 {
		t.Fatalf("two-row navigation selected %d", navigated.MessageMenu.Selected)
	}
	closed, commands := Reduce(navigated, ActionReceived{Action: Activate})
...
func TestReplyTargetClearsWhenTargetDisappears(t *testing.T) {
	...
	got, _ := Reduce(state, TelegramEvent{Value: telegram.MessageUpserted{Message: domain.Message{ID: 999, ChatID: 9, Kind: domain.MessageText, SentAt: time.Unix(999, 0)}}})
...
func TestReplyTargetLifecycleAndSendReply(t *testing.T) {
	...
	replying, _ := Reduce(state, ActionReceived{Action: ReplyMessage})
	...
	cancelled, _ := Reduce(replying, ActionReceived{Action: Close})
	...
	submitted, commands := Reduce(replying, ActionReceived{Action: ComposerSubmit, At: time.Unix(1, 0)})
...
func TestComposerEscapeCancelsReplyThenDraftThenLeaves(t *testing.T) {
	...
	withoutReply, _ := Reduce(state, ActionReceived{Action: Close})
	...
	withoutDraft, _ := Reduce(withoutReply, ActionReceived{Action: Close})
	if withoutDraft.Drafts[9] != "" || withoutDraft.Focus != FocusComposer {
		t.Fatal("second escape did not cancel draft only")
	}
	leftComposer, _ := Reduce(withoutDraft, ActionReceived{Action: Close})
...
func TestReadyRefreshesChatsWithoutDiscardingCache(t *testing.T) {
	...
	got, commands := Reduce(state, TelegramEvent{Value: telegram.Ready{}, ReceivedAt: time.Unix(1, 0)})
	...
	assertCommands(t, commands, []Command{LoadChats{RequestID: 10, Cursor: telegram.ChatCursor{Limit: 50}}})
}
...
func TestChatsLoadedSortsPreservesSelectionAndStartsHistoryAndAvatars(t *testing.T) {
	...
	assertCommands(t, commands, []Command{
...
func TestStaleOldChatMessagesFillCacheWithoutActiveSideEffects(t *testing.T) {
	...
	got, commands := Reduce(state, MessagesLoaded{
		RequestID: 11,
		ChatID:    1,
	...
	failed, commands := Reduce(state, MessagesLoadFailed{RequestID: 11, ChatID: 1, Error: failure})
...
func TestSelectingCachedOldChatRequestsItsMissingSenderAvatars(t *testing.T) {
	...
	got, commands := Reduce(state, ActionReceived{Action: SelectChat, ChatID: 1})
	if got.SelectedChat != 0 || !got.Avatars["old-sender:message-group"].Loading {
		t.Fatalf("selected cached chat = %#v", got)
	}
	assertCommands(t, commands, []Command{CloseChatCommand{ChatID: 2}, OpenChatCommand{ChatID: 1}, RenderAvatar{Key: "old-sender:message-group", Ref: ref, Role: avatar.RoleMessageGroup}})
}
...
func TestOptimisticSendQueueFailureAndCooldownRetry(t *testing.T) {
	...
	submitted, commands := Reduce(state, ActionReceived{Action: ComposerSubmit, At: now})
	...
	failed, _ := Reduce(submitted, TextQueueFailed{RequestID: 7, LocalID: -1, Error: failure, FailedAt: now})
	...
	before, commands := Reduce(failed, ActionReceived{Action: Retry, MessageID: -1, At: now.Add(4 * time.Second)})
	if before.Messages[9][0].SendState != domain.SendFailed || len(commands) != 0 {
		t.Fatalf("early retry = (%#v, %#v)", before.Messages[9][0], commands)
	}
	retried, commands := Reduce(failed, ActionReceived{Action: Retry, MessageID: -1, At: now.Add(5 * time.Second)})
	...
	assertCommands(t, commands, []Command{SendText{RequestID: 9, LocalID: -1, ChatID: 9, Text: "hello"}})
}
...
func TestQueuedTemporaryReplyIdentitySurvivesFailureAndSuccessReplacement(t *testing.T) {
	...
	queued, _ := Reduce(state, TextQueued{RequestID: 3, LocalID: -1, Message: temporary})
	...
	failed, _ := Reduce(queued, TelegramEvent{Value: telegram.MessageSendFailed{OldID: -44, Message: temporary, Error: failure}, ReceivedAt: now})
	...
	succeeded, _ := Reduce(failed, TelegramEvent{Value: telegram.MessageSendSucceeded{OldID: -44, Message: durable}, ReceivedAt: now.Add(time.Second)})
...
func TestQueuedTemporaryMessageThenTDLibFailureAndSuccessReplaceAtomically(t *testing.T) {
	...
	failedMessage.Text = ""
	failed, _ := Reduce(queued, TelegramEvent{Value: telegram.MessageSendFailed{OldID: -44, Message: failedMessage, Error: failure}, ReceivedAt: now})
...
func TestPaginationUsesPerChatNewestRelativeOffsets(t *testing.T) {
	...
	paged, commands := Reduce(state, ActionReceived{Action: PageUp})
	...
	assertCommands(t, commands, []Command{LoadMessages{RequestID: 20, ChatID: 9, Cursor: telegram.MessageCursor{FromMessageID: 1, Limit: 50}}})

	loaded, _ := Reduce(paged, MessagesLoaded{RequestID: 20, ChatID: 9, Page: telegram.MessagePage{Done: true, Messages: []domain.Message{{ID: -1, ChatID: 9, SentAt: time.Unix(-1, 0)}, {ID: 1, ChatID: 9, SentAt: time.Unix(1, 0)}}}})
	...
	down, _ := Reduce(loaded, ActionReceived{Action: PageDown})
...
func TestMessageUpsertUpdatesPreviewAndKeepsScrolledViewportAnchored(t *testing.T) {
	...
	got, commands := Reduce(state, TelegramEvent{Value: telegram.MessageUpserted{Message: incoming}, ReceivedAt: time.Unix(3, 0)})
	if got.History[9].ViewOffset != 5 || got.Chats[0].LastMessage != "new" || got.Chats[0].LastMessageAt != 2 {
		t.Fatalf("updated state = %#v", got)
	}
	assertCommands(t, commands, []Command{RenderAvatar{Key: "sender:message-group", Ref: incoming.SenderAvatar, Role: avatar.RoleMessageGroup}})
}
...
func TestDetailsAvatarModalOpenFailureAndRetry(t *testing.T) {
	...
	assertCommands(t, commands, []Command{OpenAvatar{RequestID: 30, Title: "Mina", Ref: state.Chats[0].Avatar}})

	failure := domain.AppError{Kind: domain.ErrorMedia, Message: "try again"}
	failed, _ := Reduce(opened, AvatarOpenFailed{RequestID: 30, Error: failure})
	if failed.Modal == nil || failed.Modal.Loading || failed.Modal.Error == nil {
		t.Fatalf("failed modal = %#v", failed.Modal)
	}
	retried, commands := Reduce(failed, ActionReceived{Action: Retry})
	...
	assertCommands(t, commands, []Command{OpenAvatar{RequestID: 31, Title: "Mina", Ref: state.Chats[0].Avatar}})

	ready, _ := Reduce(retried, AvatarOpened{RequestID: 31, Title: "Mina", Path: "/tmp/mina.jpg"})
	if ready.Modal.Loading || ready.Modal.Path != "/tmp/mina.jpg" || ready.Modal.Error != nil {
		t.Fatalf("ready modal = %#v", ready.Modal)
	}
	stale, _ := Reduce(ready, AvatarOpened{RequestID: 30, Title: "old", Path: "/tmp/old.jpg"})
...
func TestAvatarRenderFailureUsesPlaceholderAndRetryRetainsRequest(t *testing.T) {
	...
	failed, _ := Reduce(state, AvatarRenderFailed{Key: "mina:chat-list", Error: failure})
	...
	retried, commands := Reduce(failed, ActionReceived{Action: Retry, AvatarKey: "mina:chat-list"})
	...
	assertCommands(t, commands, []Command{RenderAvatar{Key: "mina:chat-list", Ref: ref, Role: avatar.RoleChatList}})
}
...
func TestStartupFailureQuitsOnceAndLaterInputIsIgnored(t *testing.T) {
	...
	failed, commands := Reduce(state, StartupFailed{Error: failure})
	if !failed.Quitting || failed.Fatal == nil || failed.Fatal.Message != "configure account" {
		t.Fatalf("failed state = %#v", failed)
	}
	assertCommands(t, commands, []Command{BeginShutdown{}})
	again, commands := Reduce(failed, ActionReceived{Action: Quit, Rune: 'x'})
...
func TestChatLoadFailureAndStaleResultsRetainCachedChats(t *testing.T) {
	...
	stale, _ := Reduce(state, ChatsLoadFailed{RequestID: 7, Error: failure})
	if !stale.ChatsLoading || stale.ChatsError != nil {
		t.Fatalf("stale failure changed indicators = %#v", stale)
	}
	current, _ := Reduce(state, ChatsLoadFailed{RequestID: 8, Error: failure})
	if current.ChatsLoading || current.ChatsError == nil || current.Chats[0].Title != "cached" {
		t.Fatalf("current failure state = %#v", current)
	}
	ignored, _ := Reduce(current, ChatsLoaded{RequestID: 7, Page: telegram.ChatPage{Chats: []domain.Chat{{ID: 99}}}})
...
func TestChatUpsertSortsPreservesSelectedIDAndRequestsOnlyNewAvatar(t *testing.T) {
	...
	got, commands := Reduce(state, TelegramEvent{Value: telegram.ChatUpserted{Chat: updated}})
	...
	assertCommands(t, commands, []Command{RenderAvatar{Key: "new:chat-list", Ref: updated.Avatar, Role: avatar.RoleChatList}})
	got, commands = Reduce(got, TelegramEvent{Value: telegram.ChatUpserted{Chat: updated}})
	if len(commands) != 0 {
...
func TestSelectionFocusAndCloseActionsRespectLayoutAndDrafts(t *testing.T) {
	...
	selected, commands := Reduce(state, ActionReceived{Action: SelectNext})
...
func TestUnreadAndMentionNavigationWrapsWithOrdinarySelectionEffects(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			got, gotCommands := Reduce(input, ActionReceived{Action: test.action})
			want, wantCommands := Reduce(input, ActionReceived{Action: SelectChat, ChatID: test.wantID})
...
func TestUnreadAndMentionNavigationNoMatchShowsToast(t *testing.T) {
	for _, test := range []struct {
		...
		got, commands := Reduce(state, ActionReceived{Action: test.action})
...
func TestFocusPaneAndCycleOnlyVisitVisiblePanes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			got, _ := Reduce(state, test.action)
...
func TestComposerEditingAndSubmitGuardsArePerChat(t *testing.T) {
	state := selectedWritableState()
	state.Drafts[9] = "界"
	edited, _ := Reduce(state, ActionReceived{Rune: '面'})
	edited, _ = Reduce(edited, ActionReceived{Action: ComposerNewline})
	edited, _ = Reduce(edited, ActionReceived{Action: ComposerBackspace})
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			got, commands := Reduce(candidate, ActionReceived{Action: ComposerSubmit})
...
func TestCurrentAndStaleMessageLoadFailuresUpdateOnlyMatchingHistory(t *testing.T) {
	...
	stale, _ := Reduce(state, MessagesLoadFailed{RequestID: 3, ChatID: 9, Error: failure})
	if stale.History[9] != state.History[9] || stale.Toast != nil {
		t.Fatalf("stale failure = %#v", stale)
	}
	current, _ := Reduce(state, MessagesLoadFailed{RequestID: 4, ChatID: 9, Error: failure})
...
func TestLoadedMessagesRenderEachMissingGroupAvatarOnce(t *testing.T) {
	...
	got, commands := Reduce(state, MessagesLoaded{RequestID: 5, ChatID: 9, Page: page})
	if len(commands) != 1 {
		t.Fatalf("avatar commands = %#v", commands)
	}
	assertCommands(t, commands, []Command{RenderAvatar{Key: "iris:message-group", Ref: ref, Role: avatar.RoleMessageGroup}})
...
func TestActivateFailedSelectedMessageHonorsCooldownWithoutChangingFocus(t *testing.T) {
	...
	early, commands := Reduce(state, ActionReceived{Action: Activate, At: now})
	...
	ready, commands := Reduce(state, ActionReceived{Action: Activate, At: now.Add(time.Second)})
...
func TestResizeClampsEachChatOffsetAndMessageLimitPreservesOutgoingFailures(t *testing.T) {
	...
	resized, _ := Reduce(state, Resized{Width: 100, Height: 22})
	...
	limited, _ := Reduce(state, TelegramEvent{Value: telegram.MessageUpserted{Message: domain.Message{ID: 9999, ChatID: 9, SentAt: time.Unix(9999, 0)}}})
...
func TestHistoryFailureRetainsErrorAndRetryUsesOldestCursor(t *testing.T) {
	...
	failed, _ := Reduce(state, MessagesLoadFailed{RequestID: 7, ChatID: 9, Error: failure})
...
func TestAvatarFailurePlaceholderUsesDisplayLabel(t *testing.T) {
	...
	got, _ := Reduce(state, AvatarRenderFailed{Key: key, Error: domain.AppError{Kind: domain.ErrorMedia}})
...
func TestPaginationBeyondBoundKeepsNewlyLoadedOlderPage(t *testing.T) {
	...
	got, _ := Reduce(state, MessagesLoaded{RequestID: 7, ChatID: 9, Page: telegram.MessagePage{Messages: older, Done: false}})
...
func TestSendResultReplacementIsScopedToMessageChat(t *testing.T) {
	...
	succeeded, _ := Reduce(state, TelegramEvent{Value: telegram.MessageSendSucceeded{
		OldID:   -30,
	...
	failed, _ := Reduce(state, TelegramEvent{Value: telegram.MessageSendFailed{
		OldID:   -30,
...
func TestMessageEditLifecyclePreservesDraftAndRetriesWithoutOptimisticMutation(t *testing.T) {
	...
	state.SelectedMessageChat, state.SelectedMessage = 9, 2
	opened, commands := Reduce(state, ActionReceived{Action: EditMessage})
	if opened.MessageMenu == nil || !opened.MessageMenu.Loading || len(commands) != 1 {
		t.Fatal("edit shortcut did not open authoritative menu")
	}
	request := commands[0].(GetMessageProperties)
	loaded, _ := Reduce(opened, MessagePropertiesLoaded{RequestID: request.RequestID, ChatID: 9, MessageID: 2, Capabilities: domain.MessageCapabilities{Copy: true, Edit: true}})
	...
	editing, _ := Reduce(loaded, ActionReceived{Action: Activate})
	...
	typed, _ := Reduce(editing, ActionReceived{Action: NoAction, Rune: 'x'})
	...
	submitting, commands := Reduce(typed, ActionReceived{Action: ComposerSubmit})
	...
	failed, _ := Reduce(submitting, TextEditFailed{RequestID: edit.RequestID, ChatID: 9, MessageID: 2, Error: domain.AppError{Message: "secret"}})
	...
	retry, commands := Reduce(failed, ActionReceived{Action: ComposerSubmit})
	if retry.EditTarget == nil || len(commands) != 1 {
		t.Fatal("retry unavailable")
	}
	stale, _ := Reduce(retry, TextEdited{RequestID: edit.RequestID, ChatID: 9, MessageID: 2, Message: domain.Message{ID: 2, ChatID: 9, Text: "stale"}})
...
func TestMessageEditCancelGuardsAndSuccessMergeMetadata(t *testing.T) {
	...
	unchanged, commands := Reduce(state, ActionReceived{Action: ComposerSubmit})
	if len(commands) != 0 || unchanged.EditTarget == nil {
		t.Fatal("unchanged reached transport")
	}
	unchanged.EditTarget.Buffer = "  \n"
	empty, commands := Reduce(unchanged, ActionReceived{Action: ComposerSubmit})
	if len(commands) != 0 || empty.EditTarget == nil {
		t.Fatal("empty reached transport")
	}
	canceled, _ := Reduce(empty, ActionReceived{Action: Close})
	...
	submitting, commands := Reduce(state, ActionReceived{Action: ComposerSubmit})
	request := commands[0].(EditText)
	success, _ := Reduce(submitting, TextEdited{RequestID: request.RequestID, ChatID: 9, MessageID: 2, Message: domain.Message{ID: 2, ChatID: 9, Kind: domain.MessageText, Text: "changed", EditedAt: time.Unix(20, 0)}})
...
func TestMessageEditCancelsOnChatSwitchExternalChangeAndDisappearance(t *testing.T) {
	...
	switched, _ := Reduce(base, ActionReceived{Action: SelectChat, ChatID: 10})
	if switched.EditTarget != nil {
		t.Fatal("chat switch retained edit target")
	}
	externallyChanged, _ := Reduce(base, TelegramEvent{Value: telegram.MessageContentUpdated{ChatID: 9, MessageID: 2, Kind: domain.MessageText, Text: "opaque-external"}})
	...
	evicted, _ := Reduce(base, TelegramEvent{Value: telegram.MessageUpserted{Message: domain.Message{ID: 999, ChatID: 9, Kind: domain.MessageText, SentAt: time.Unix(999, 0)}}})
	...
	base.Messages[9] = []domain.Message{{ID: 2, ChatID: 9, Kind: domain.MessageText, Text: "opaque-original"}}
	base.EditTarget = &EditTarget{ChatID: 9, MessageID: 2, Original: "opaque-original", Buffer: "opaque-buffer"}
	replaced, _ := Reduce(base, TelegramEvent{Value: telegram.MessageUpserted{Message: domain.Message{ID: 2, ChatID: 9, Kind: domain.MessagePhoto}}})
	...
	base.Messages[9] = []domain.Message{{ID: 2, ChatID: 9, Kind: domain.MessageText, Text: "opaque-original"}}
	base.EditTarget = &EditTarget{ChatID: 9, MessageID: 2, Original: "opaque-original", Buffer: "opaque-buffer"}
	deleted, _ := Reduce(base, TelegramEvent{Value: telegram.MessagesDeleted{ChatID: 9, MessageIDs: []domain.MessageID{2}}})
	...
	base.EditTarget = &EditTarget{ChatID: 9, MessageID: 2, Original: "opaque-original", Buffer: "opaque-buffer"}
	evicted, _ = Reduce(base, TelegramEvent{Value: telegram.MessagesDeleted{ChatID: 9, MessageIDs: []domain.MessageID{2}, FromCache: true}})
...
func TestDeleteMessageActivatesWithinModalUsesSavedIdentityAndRevokeFalse(t *testing.T) {
	...
	deleting, commands := Reduce(state, ActionReceived{Action: DeleteMessage})
	if len(commands) != 1 {
...
func TestDeleteMessageSuccessRemovesExactMessageReconcilesSelectionAndClearsTargets(t *testing.T) {
	...
	deleted, _ := Reduce(deleting, MessageDeleted{RequestID: request, ChatID: 9, MessageID: 2})
...
func TestDeleteMessageSuccessWhenMessageAlreadyAbsentIsIdempotent(t *testing.T) {
	...
	deleting, _ := Reduce(state, MessageDeleted{RequestID: 77, ChatID: 9, MessageID: 2})
...
func TestDeleteMessageFailureRetainsMessageAndShowsConstantToast(t *testing.T) {
	...
	deleting, commands := Reduce(state, ActionReceived{Action: DeleteMessage})
	request := commands[0].(DeleteMessageCommand).RequestID
	failed, _ := Reduce(deleting, MessageDeleteFailed{RequestID: request, ChatID: 9, MessageID: 2, Error: domain.AppError{Message: "raw secret-body 9 2"}})
...
func TestDeleteForEveryoneIssuesRevokeDeleteAndRequiresDeleteForAll(t *testing.T) {
	...
	_, commands := Reduce(state, ActionReceived{Action: DeleteForEveryone})
	if len(commands) != 1 {
	...
	state.MessageMenu = &MessageActionMenu{ChatID: 9, MessageID: 2, Capabilities: domain.MessageCapabilities{DeleteForSelf: true}}
	selfOnly, commands := Reduce(state, ActionReceived{Action: DeleteForEveryone})
	if len(commands) != 0 {
	...
	state.MessageMenu = &MessageActionMenu{ChatID: 9, MessageID: 2, Capabilities: domain.MessageCapabilities{DeleteForAll: true}}
	forAllOnly, commands := Reduce(state, ActionReceived{Action: DeleteMessage})
	if len(commands) != 0 {
...
func TestDeleteMessageEventsIgnoreStaleAndIdentityMismatch(t *testing.T) {
	...
	stale, _ := Reduce(state, MessageDeleted{RequestID: 98, ChatID: 9, MessageID: 2})
	if stale.MessageMenu == nil || messageIndex(stale.Messages[9], 2) < 0 {
		t.Fatal("stale success applied")
	}
	// Identity mismatch on failure.
	mismatched, _ := Reduce(state, MessageDeleteFailed{RequestID: 99, ChatID: 10, MessageID: 2, Error: domain.AppError{Message: "raw 10 2"}})
	...
	wrongMessage, _ := Reduce(state, MessageDeleted{RequestID: 99, ChatID: 9, MessageID: 3})
	...
	closed, _ := Reduce(state, ActionReceived{Action: Close})
	late, _ := Reduce(closed, MessageDeleted{RequestID: 99, ChatID: 9, MessageID: 2})
...
func TestDeleteForEveryoneHasSingleRowOrderAndNoOptimisticRemoval(t *testing.T) {
	...
	deleting, commands := Reduce(state, ActionReceived{Action: DeleteForEveryone})
...
func TestPinMessageActivationResolvesUnpinFromMenuPinned(t *testing.T) {
	...
	pinning, commands := Reduce(state, ActionReceived{Action: PinMessage})
	if len(commands) != 1 {
	...
	unpinning, commands := Reduce(state, ActionReceived{Action: PinMessage})
...
func TestPinMessageRequiresCapabilityAndLivingIdentity(t *testing.T) {
	...
	gated, commands := Reduce(state, ActionReceived{Action: PinMessage})
	...
	missing, commands := Reduce(state, ActionReceived{Action: PinMessage})
	if len(commands) != 0 {
...
func TestPinMessageSuccessSetsPinnedAndShowsToast(t *testing.T) {
	...
	changed, _ := Reduce(pinning, MessagePinChanged{RequestID: request, ChatID: 9, MessageID: 2, Pinned: true})
	...
	unpinning, commands := Reduce(state, ActionReceived{Action: PinMessage})
	request = commands[0].(PinMessageCommand).RequestID
	unpinned, _ := Reduce(unpinning, MessagePinChanged{RequestID: request, ChatID: 9, MessageID: 2, Pinned: false})
...
func TestPinMessageFailureShowsConstantToastAndClosesMenu(t *testing.T) {
	...
	pinning, commands := Reduce(state, ActionReceived{Action: PinMessage})
	request := commands[0].(PinMessageCommand).RequestID
	failed, _ := Reduce(pinning, MessagePinFailed{RequestID: request, ChatID: 9, MessageID: 2, Error: domain.AppError{Message: "raw secret-body 9 2"}})
	...
	unpinning, commands := Reduce(state, ActionReceived{Action: PinMessage})
	request = commands[0].(PinMessageCommand).RequestID
	unpinFailed, _ := Reduce(unpinning, MessagePinFailed{RequestID: request, ChatID: 9, MessageID: 2, Error: domain.AppError{Message: "raw"}})
...
func TestPinMessageEventsIgnoreStaleAndIdentityMismatch(t *testing.T) {
	...
	stale, _ := Reduce(state, MessagePinChanged{RequestID: 98, ChatID: 9, MessageID: 2, Pinned: true})
	if stale.MessageMenu == nil || stale.Messages[9][0].Pinned {
		t.Fatal("stale success applied")
	}
	mismatched, _ := Reduce(state, MessagePinFailed{RequestID: 99, ChatID: 10, MessageID: 2, Error: domain.AppError{Message: "raw 10 2"}})
	if mismatched.Toast != nil || mismatched.MessageMenu == nil {
		t.Fatal("identity-mismatched failure applied")
	}
	wrongMessage, _ := Reduce(state, MessagePinChanged{RequestID: 99, ChatID: 9, MessageID: 3, Pinned: true})
	if wrongMessage.MessageMenu == nil || wrongMessage.Messages[9][0].Pinned {
		t.Fatal("identity-mismatched success applied")
	}
	closed, _ := Reduce(state, ActionReceived{Action: Close})
	late, _ := Reduce(closed, MessagePinChanged{RequestID: 99, ChatID: 9, MessageID: 2, Pinned: true})
...
func TestMessagePinnedUpdatedReconcilesInPlace(t *testing.T) {
	...
	state.Chats = []domain.Chat{{ID: 9}}
	state.Messages[9] = []domain.Message{{ID: 2, ChatID: 9, Kind: domain.MessageText}}
	got, _ := Reduce(state, TelegramEvent{Value: telegram.MessagePinnedUpdated{ChatID: 9, MessageID: 2, Pinned: true}})
	if !got.Messages[9][0].Pinned {
		t.Fatal("external pin update not applied in place")
	}
	got, _ = Reduce(got, TelegramEvent{Value: telegram.MessagePinnedUpdated{ChatID: 9, MessageID: 2, Pinned: false}})
	if got.Messages[9][0].Pinned {
	...
	got, _ = Reduce(state, TelegramEvent{Value: telegram.MessagePinnedUpdated{ChatID: 9, MessageID: 99, Pinned: true}})
	if got.Messages[9][0].Pinned {
...
func TestForwardMenuActivationVerifiesIdentityAndOpensPicker(t *testing.T) {
	...
	opened, commands := Reduce(state, ActionReceived{Action: ForwardMessageSource})
	if len(commands) != 0 {
...
func TestForwardMenuActivationRequiresCapabilityAndLivingIdentity(t *testing.T) {
	...
	state.Messages[9] = []domain.Message{{ID: 2, ChatID: 9, Kind: domain.MessageText}}
	state.MessageMenu = &MessageActionMenu{ChatID: 9, MessageID: 2, Capabilities: domain.MessageCapabilities{Copy: true}, PreviousFocus: FocusConversation}
	gated, _ := Reduce(state, ActionReceived{Action: ForwardMessageSource})
	...
	missing, _ := Reduce(state, ActionReceived{Action: ForwardMessageSource})
...
func TestForwardPickerNavigationAndConfirmEmitsExactCommand(t *testing.T) {
	...
	next, _ := Reduce(state, ActionReceived{Action: SelectNext})
...
func TestForwardPickerCancelRestoresFocusAndClearsPicker(t *testing.T) {
	...
	got, _ := Reduce(state, ActionReceived{Action: Close})
...
func TestForwardSuccessClosesPickerShowsToastAndDoesNotOptimisticallyInsert(t *testing.T) {
	...
	got, _ := Reduce(state, MessageForwarded{RequestID: 42, DestinationChatID: 10})
	if got.ForwardPicker != nil || got.Focus != FocusConversation {
...
func TestForwardFailureClosesPickerAndShowsConstantToast(t *testing.T) {
	...
	state.Messages[9] = []domain.Message{{ID: 2, ChatID: 9, Kind: domain.MessageText, Text: "secret-body"}}
	state.ForwardPicker = &ForwardPicker{SourceChatID: 9, SourceMessageID: 2, SelectedChat: 1, RequestID: 42}
	got, _ := Reduce(state, MessageForwardFailed{RequestID: 42, DestinationChatID: 10, Error: domain.AppError{Message: "raw secret-body 9 10 42"}})
	if got.ForwardPicker != nil || got.Focus != FocusConversation {
...
func TestForwardPickerEventsIgnoreStaleAndIdentityMismatch(t *testing.T) {
	...
	stale, _ := Reduce(state, MessageForwarded{RequestID: 41, DestinationChatID: 10})
	if stale.ForwardPicker == nil || stale.Toast != nil {
		t.Fatal("stale success applied")
	}
	wrongDestination, _ := Reduce(state, MessageForwardFailed{RequestID: 42, DestinationChatID: 9, Error: domain.AppError{Message: "raw"}})
	if wrongDestination.ForwardPicker == nil || wrongDestination.Toast != nil {
		t.Fatal("identity-mismatched failure applied")
	}
	late, _ := Reduce(state, ActionReceived{Action: Close})
	late, _ = Reduce(late, MessageForwarded{RequestID: 42, DestinationChatID: 10})
	if late.Toast != nil {
...
func TestForwardPickerClearedOnChatSwitchAndSourceDisappearance(t *testing.T) {
	...
	state.ForwardPicker = &ForwardPicker{SourceChatID: 9, SourceMessageID: 2, SelectedChat: 0, RequestID: 42}
	deleted, _ := Reduce(state, TelegramEvent{Value: telegram.MessagesDeleted{ChatID: 9, MessageIDs: []domain.MessageID{2}}})
	...
	evicted, _ := Reduce(state, TelegramEvent{Value: telegram.MessageUpserted{Message: domain.Message{ID: 999, ChatID: 9, Kind: domain.MessageText, SentAt: time.Unix(999, 0)}}})
...
func TestReactMenuActivationVerifiesIdentityAndOpensPicker(t *testing.T) {
	...
	opened, commands := Reduce(state, ActionReceived{Action: ReactMessage})
	if len(commands) != 0 {
		t.Fatalf("picker open commands = %d, want 0", len(commands))
	}
	if opened.MessageMenu != nil {
	...
	gated, _ = Reduce(gated, ActionReceived{Action: ReactMessage})
	...
	loading, _ = Reduce(loading, ActionReceived{Action: ReactMessage})
	...
	missing, _ = Reduce(missing, ActionReceived{Action: ReactMessage})
...
func TestReactionPickerNavigationConfirmAndCancel(t *testing.T) {
	...
	cancelled, _ := Reduce(state, ActionReceived{Action: Close})
...
func TestReactionPickerToggleAddVsRemoveFromChosen(t *testing.T) {
	...
	_, commands := Reduce(state, ActionReceived{Action: Activate})
	...
	_, commands = Reduce(chosenOther, ActionReceived{Action: Activate})
	...
	_, commands = Reduce(chosenFalse, ActionReceived{Action: Activate})
...
func TestReactionPickerIdentityIsExactAndCommandUsesSavedIdentity(t *testing.T) {
	...
	confirmed, commands := Reduce(state, ActionReceived{Action: Activate})
...
func TestReactionSuccessClosesPickerShowsConstantToastWithoutOptimisticMutation(t *testing.T) {
	...
	added, _ := Reduce(state, ReactionChanged{RequestID: 5, ChatID: 9, MessageID: 2, Emoji: "👍", Removed: false})
	...
	removed, _ := Reduce(removedState, ReactionChanged{RequestID: 6, ChatID: 9, MessageID: 2, Emoji: "👍", Removed: true})
...
func TestReactionFailureShowsConstantToastNoCauseAndClosesPicker(t *testing.T) {
	...
	state.Messages[9] = []domain.Message{{ID: 2, ChatID: 9, Kind: domain.MessageText, Text: "secret-body"}}
	state.ReactionPicker = &ReactionPicker{ChatID: 9, MessageID: 2, RequestID: 5}
	failed, _ := Reduce(state, ReactionFailed{RequestID: 5, ChatID: 9, MessageID: 2, Error: domain.AppError{Message: "secret-body 9 2"}})
...
func TestReactionEventsIgnoreStaleAndIdentityMismatch(t *testing.T) {
	...
	stale, _ := Reduce(state, ReactionChanged{RequestID: 41, ChatID: 9, MessageID: 2, Emoji: "👍"})
	if stale.ReactionPicker == nil || stale.Toast != nil {
		t.Fatal("stale success applied")
	}
	wrongChat, _ := Reduce(state, ReactionFailed{RequestID: 42, ChatID: 10, MessageID: 2, Error: domain.AppError{Message: "raw"}})
	if wrongChat.ReactionPicker == nil || wrongChat.Toast != nil {
		t.Fatal("identity-mismatched failure applied")
	}
	wrongMessage, _ := Reduce(state, ReactionChanged{RequestID: 42, ChatID: 9, MessageID: 3, Emoji: "👍"})
	if wrongMessage.ReactionPicker == nil || wrongMessage.Toast != nil {
		t.Fatal("identity-mismatched success applied")
	}
	closed, _ := Reduce(state, ActionReceived{Action: Close})
	late, _ := Reduce(closed, ReactionChanged{RequestID: 42, ChatID: 9, MessageID: 2, Emoji: "👍"})
	if late.Toast != nil {
		t.Fatal("late success applied after picker closed")
	}
}
...
func TestUpdateMessageReactionsReconcilesInPlaceAndIgnoresUnknownIdentity(t *testing.T) {
	...
	got, _ := Reduce(state, TelegramEvent{Value: telegram.MessageReactionsUpdated{ChatID: 9, MessageID: 2, Reactions: reactions}})
	...
	got, _ = Reduce(state, TelegramEvent{Value: telegram.MessageReactionsUpdated{ChatID: 9, MessageID: 99, Reactions: reactions}})
...
func TestMessageReactionsSurviveReUpsertWithEmptySnapshot(t *testing.T) {
	...
	live, _ := Reduce(state, TelegramEvent{Value: telegram.MessageReactionsUpdated{
		ChatID:    9,
		MessageID: 2,
	...
	reupserted, _ := Reduce(live, TelegramEvent{Value: telegram.MessageUpserted{
...
func TestReducerReplacesAndClearsMessageMediaOnContentUpdate(t *testing.T) {
	...
	state, commands = Reduce(state, TelegramEvent{Value: telegram.MessageContentUpdated{
		ChatID:    9,
		MessageID: 10,
...
func TestMediaModalOpensWithDownloadCommandAndSelectionReset(t *testing.T) {
	...
	open, commands := Reduce(state, ActionReceived{Action: ViewMessageMedia})
...
func TestMediaModalRetryReusesDownloadMediaCommand(t *testing.T) {
	...
	failure := domain.AppError{Kind: domain.ErrorNetwork, Message: "offline"}
	failed, _ := Reduce(state, MessageMediaOpenFailed{RequestID: 20, ChatID: 9, MessageID: 2, Error: failure})
...
func TestMediaModalIdentityMatchesAndRejectsStaleEvents(t *testing.T) {
	...
	matched, _ := Reduce(state, MessageMediaOpened{RequestID: 30, ChatID: 9, MessageID: 2, Title: "Photo", File: domain.MediaFileRef{ID: 101, Downloaded: true, LocalPath: "/tmp/101.jpg"}})
	...
	stale, _ := Reduce(fresh, MessageMediaOpened{RequestID: 29, ChatID: 9, MessageID: 2, Title: "Photo", File: domain.MediaFileRef{ID: 101}})
	if !stale.Modal.Loading {
		t.Fatal("stale event should not have matched")
	}
	wrongChat, _ := Reduce(fresh, MessageMediaOpened{RequestID: 30, ChatID: 8, MessageID: 2, Title: "Photo", File: domain.MediaFileRef{ID: 101}})
	if !wrongChat.Modal.Loading {
		t.Fatal("wrong chat ID should not have matched")
	}
	wrongMessage, _ := Reduce(fresh, MessageMediaOpened{RequestID: 30, ChatID: 9, MessageID: 3, Title: "Photo", File: domain.MediaFileRef{ID: 101}})
...
func TestMediaModalSuccessUpdatesLocalMessagePath(t *testing.T) {
	...
	updated, _ := Reduce(state, MessageMediaOpened{RequestID: 40, ChatID: 9, MessageID: 2, Title: "Photo", File: domain.MediaFileRef{ID: 101, Downloaded: true, LocalPath: "/tmp/101.jpg"}})
...
func TestMediaModalFailureDoesNotClearMessageMediaRef(t *testing.T) {
	...
	failed, _ := Reduce(state, MessageMediaOpenFailed{RequestID: 50, ChatID: 9, MessageID: 2, Error: failure})
	if failed.Modal.Loading || failed.Modal.Error == nil {
		t.Fatalf("failed modal = %#v", failed.Modal)
	}
	message := failed.Messages[9][0]
...
func TestMediaModalMatchesOnUniqueIDWhenIDIsZero(t *testing.T) {
	...
	matched, _ := Reduce(state, MessageMediaOpened{RequestID: 60, ChatID: 9, MessageID: 2, Title: "Photo", File: domain.MediaFileRef{ID: 0, UniqueID: "unique-101", Downloaded: true, LocalPath: "/tmp/101.jpg"}})
	if matched.Modal.Loading || matched.Modal.Path != "/tmp/101.jpg" {
		t.Fatalf("matched modal = %#v", matched.Modal)
	}
	stale, _ := Reduce(state, MessageMediaOpened{RequestID: 60, ChatID: 9, MessageID: 2, Title: "Photo", File: domain.MediaFileRef{ID: 0, UniqueID: "unique-202"}})
	if !stale.Modal.Loading {
...
func TestMediaModalRetryPreservesIdentityMatchOnUniqueID(t *testing.T) {
	...
	failure := domain.AppError{Kind: domain.ErrorMedia, Message: "try again"}
	failed, _ := Reduce(state, MessageMediaOpenFailed{RequestID: 70, ChatID: 9, MessageID: 2, Error: failure})
	retried, commands := Reduce(failed, ActionReceived{Action: Retry})
...
func TestMediaModalMatchesRejectsWhenModalIsNil(t *testing.T) {
	...
	state.Focus = FocusModal
	state.Modal = nil
	matched, _ := Reduce(state, MessageMediaOpened{RequestID: 90, ChatID: 9, MessageID: 2, Title: "Photo", File: domain.MediaFileRef{ID: 101}})
...
func TestMediaModalIdentityMatchesRejectsWrongRequestID(t *testing.T) {
	...
	result, _ := Reduce(state, MessageMediaOpened{RequestID: 99, ChatID: 9, MessageID: 2, Title: "Photo", File: domain.MediaFileRef{ID: 101, Downloaded: true}})
...
func TestMessagePhotoActionOpensReadyModalFromCompletedLocalFile(t *testing.T) {
	...
	opened, commands := Reduce(state, ActionReceived{Action: ViewMessageMedia})
...
func TestMessagePhotoActionDownloadsAndUsesMenuOwnedIdentity(t *testing.T) {
	...
	wantCommands := []Command{OpenMessageMediaFile{
		RequestID: 10,
		ChatID:    9,
		MessageID: 2,
		Title:     "Photo",
	...
	result, commands2 := Reduce(opened, MessageMediaOpened{
	...
	no1, cmds1 := Reduce(buildIdentityMismatchState(), ActionReceived{Action: ViewMessageMedia})
	...
	no2, cmds2 := Reduce(buildMissingState(), ActionReceived{Action: ViewMessageMedia})
	...
	no3, cmds3 := Reduce(buildPhotoTextState(), ActionReceived{Action: ViewMessageMedia})
	...
	successResult, successCmds := Reduce(buildSuccessState(), MessageMediaOpened{
	...
	noInv1, invCmds1 := Reduce(buildInvSuccess1(), MessageMediaOpened{
	...
	noInv2, invCmds2 := Reduce(buildInvSuccess2(), MessageMediaOpened{
...
func TestMessageMediaDownloadSuccessUpdatesOnlyMatchingAvailability(t *testing.T) {
	...
	result, cmds := Reduce(state, MessageMediaOpened{
...
func TestMessageMediaDownloadFailureRetryAndStaleResults(t *testing.T) {
	...
	failed, cmds := Reduce(state, MessageMediaOpenFailed{
	...
	retried, retryCmds := Reduce(buildRetryState(), ActionReceived{Action: Retry})
	...
	wantRetryCmd := []Command{OpenMessageMediaFile{
	...
	successStaleReq, cmdsStaleReq := Reduce(retried, MessageMediaOpened{
	...
	successStaleChat, cmdsStaleChat := Reduce(retried, MessageMediaOpened{
	...
	successStaleMsg, cmdsStaleMsg := Reduce(retried, MessageMediaOpened{
	...
	successStaleFile, cmdsStaleFile := Reduce(retried, MessageMediaOpened{
		RequestID: 200, ChatID: 9, MessageID: 2, Title: "Photo",
	...
	sf1, cmdsSF1 := Reduce(buildStaleFailureState(), MessageMediaOpenFailed{
	...
	sf2, cmdsSF2 := Reduce(buildStaleFailureState(), MessageMediaOpenFailed{
	...
	sf3, cmdsSF3 := Reduce(buildStaleFailureState(), MessageMediaOpenFailed{
	...
	avatarRetried, avatarRetryCmds := Reduce(avatarRetryState, ActionReceived{Action: Retry})
	...
	wantAvatarCmd := []Command{OpenAvatar{RequestID: 300, Title: "Mina", Ref: domain.AvatarRef{UniqueID: "small", OriginalUniqueID: "large"}}}
...
func TestPhotoSendOpenGuards(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			state := InitialState()
			tc.configure(&state)
			result, _ := Reduce(state, ActionReceived{Action: OpenPhotoSend})
			if tc.name == "valid" {
...
func TestPhotoSendInputCancelPreservesDraftReplyAndPendingLedger(t *testing.T) {
	...
	state, _ = Reduce(state, ActionReceived{Action: OpenPhotoSend})
	entered, _ := Reduce(state, ActionReceived{Action: NoAction, Rune: '界'})
	entered, _ = Reduce(entered, ActionReceived{Action: NoAction, Rune: '\r'})
	entered, _ = Reduce(entered, ActionReceived{Action: NoAction, Rune: '\n'})
	...
	backed, _ := Reduce(entered, ActionReceived{Action: ComposerBackspace})
...
func TestPhotoSendSubmitExactOptimisticCommand(t *testing.T) {
	...
	submitted, cmds := Reduce(state, ActionReceived{Action: PhotoSendSubmit, At: now})
	...
	stateNil, _ = Reduce(stateNil, ActionReceived{Action: OpenPhotoSend})
	stateNil.PhotoSend.Input = []rune{'/'}
	stateNil.PhotoSendRequests = nil
	_, _ = Reduce(stateNil, ActionReceived{Action: PhotoSendSubmit})
}
...
func TestPhotoSendInvalidSubmitFullStateNoOp(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			_ = before
			result, cmds := Reduce(state, ActionReceived{Action: PhotoSendSubmit})
...
func TestPhotoSendQueuedCorrelationAndSourceFallback(t *testing.T) {
	...
	got, _ := Reduce(state, PhotoQueued{RequestID: 100, LocalID: localID, ChatID: 9, Message: queued})
	...
	got2, _ := Reduce(state2, PhotoQueued{RequestID: 1, LocalID: localID, ChatID: 9, Message: queued2})
...
func TestPhotoSendQueuedInvalidEventsFullStateNoOp(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, cmds := Reduce(state, tc.event)
			if !reflect.DeepEqual(result, state) || len(cmds) != 0 {
...
func TestPhotoSendQueueFailureCorrelation(t *testing.T) {
	...
	failed, _ := Reduce(state, PhotoQueueFailed{RequestID: 100, LocalID: localID, ChatID: 9, Error: failure, FailedAt: now})
	...
	stale, _ := Reduce(state2, PhotoQueueFailed{RequestID: 99, LocalID: domain.MessageID(-8), ChatID: 9, Error: failure, FailedAt: now})
...
func TestRetryMessagePhotoExactAndInvalidFullStateNoOp(t *testing.T) {
	...
	got, cmds := Reduce(state, ActionReceived{Action: Retry, MessageID: -5, At: now})
	if len(cmds) != 1 {
		t.Fatalf("cmds = %d", len(cmds))
	}
	cmd := cmds[0].(SendPhoto)
	...
	result, cmds2 := Reduce(state2, ActionReceived{Action: Retry, MessageID: -5, At: now})
...
func TestTextRetryRegressionAfterPhotoSupport(t *testing.T) {
	...
	state.PhotoSendRequests = map[domain.MessageID]uint64{domain.MessageID(-99): 999}
	got, cmds := Reduce(state, ActionReceived{Action: Retry, MessageID: -1, At: now})
	if len(cmds) != 1 {
		t.Fatalf("cmds = %d", len(cmds))
	}
	cmd := cmds[0].(SendText)
...
func TestPhotoSendTerminalReconciliationPreservesCaptionReplyAndSource(t *testing.T) {
	...
	result, _ := Reduce(state, TelegramEvent{Value: telegram.MessageSendSucceeded{
		OldID:   -3,
	...
	result2, _ := Reduce(state2, TelegramEvent{Value: telegram.MessageSendSucceeded{
		OldID: -4,
	...
	result3, _ := Reduce(state3, TelegramEvent{Value: telegram.MessageSendFailed{
		OldID:   -5,
...
func TestThumbnailAutoDownloadTrigger(t *testing.T) {
	...
	got, commands := Reduce(state, TelegramEvent{Value: telegram.MessageUpserted{Message: message}, ReceivedAt: time.Unix(300, 0)})
	assertCommands(t, commands, []Command{DownloadThumbnail{RequestID: 1, ChatID: 9, MessageID: 2, File: domain.MediaFileRef{ID: 7, UniqueID: "thumb", CanDownload: true, Downloaded: false}}})
	msg := got.Messages[9][0]
...
func TestThumbnailDispatchWhenAlreadyDownloadedNotRendered(t *testing.T) {
	...
	assertCommands(t, commands, []Command{DownloadThumbnail{RequestID: 1, ChatID: 9, MessageID: 2, File: domain.MediaFileRef{ID: 7, CanDownload: true, Downloaded: true, LocalPath: "/tmp/thumb.jpg"}}})
	msg := got.Messages[9][0]
...
func TestThumbnailNoDispatchWhenAlreadyRendered(t *testing.T) {
	...
	_, commands := Reduce(state, TelegramEvent{Value: telegram.MessageUpserted{Message: message}, ReceivedAt: time.Unix(300, 0)})
	assertCommands(t, commands, []Command{})
}
...
func TestThumbnailDownloadedUpdatesMessage(t *testing.T) {
	...
	updated, commands := Reduce(state, ThumbnailDownloaded{RequestID: 1, ChatID: 9, MessageID: 2, File: domain.MediaFileRef{ID: 7, CanDownload: true, Downloaded: true, LocalPath: "/tmp/thumb-dl.jpg"}})
	if len(commands) != 0 {
...
func TestThumbnailDownloadFailedIsSilent(t *testing.T) {
	...
	updated, commands := Reduce(state, ThumbnailDownloadFailed{RequestID: 1, ChatID: 9, MessageID: 2, Error: domain.AppError{Kind: domain.ErrorInternal, Op: "download thumbnail", Message: "failed"}})
	if len(commands) != 0 {
		t.Fatalf("commands = %#v, want none", commands)
	}
	thumb := updated.Messages[9][0].Media.Thumbnail
...
func TestMessageContentUpdatedPreservesDownloadedThumbnailState(t *testing.T) {
	...
	updated, _ := Reduce(state, TelegramEvent{Value: telegram.MessageContentUpdated{
...
func TestThumbnailRenderedStoresBlock(t *testing.T) {
	...
	updated, _ := Reduce(state, ThumbnailRendered{RequestID: 1, ChatID: chatID, MessageID: msgID, Block: block})
...
func TestThumbnailRenderedNilMapInitialized(t *testing.T) {
	...
	updated, _ := Reduce(state, ThumbnailRendered{RequestID: 2, ChatID: chatID, MessageID: msgID, Block: block})
...
func TestAttachmentOpenAcceptance_CorrelationToastsAndStaleGuards(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.title, func(t *testing.T) {
			...
			menuState, _ := Reduce(state, ActionReceived{Action: OpenMessageActionMenu})
			...
			for _, stale := range staleEvents {
				got, staleCommands := Reduce(started, stale)
				if len(staleCommands) != 0 || !reflect.DeepEqual(got, started) {
					t.Fatalf("stale %T changed state: commands=%#v", stale, staleCommands)
				}
			}

			succeeded, successCommands := Reduce(started, MessageMediaOpened{RequestID: 41, ChatID: 9, MessageID: 77, Title: tc.title, File: openedFile})
			...
			retryMenu, _ := Reduce(succeeded, ActionReceived{Action: OpenMessageActionMenu})
			reopened, reopenedCommands := Reduce(retryMenu, ActionReceived{Action: ViewMessageMedia})
			if len(reopenedCommands) != 1 {
				t.Fatalf("reopen commands = %#v, want one", reopenedCommands)
			}
			failed, failedCommands := Reduce(reopened, MessageMediaOpenFailed{RequestID: 43, ChatID: 9, MessageID: 77, Error: domain.AppError{Kind: domain.ErrorInternal, Message: "private /tmp/path raw failure", Cause: errors.New("private cause")}})
			...
			retryMenu2, _ := Reduce(failed, ActionReceived{Action: OpenMessageActionMenu})
