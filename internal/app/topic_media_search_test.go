...
func forumPhotoSendPath(t *testing.T, state State, path string) (State, []Command) {
	t.Helper()
	...
	state, _ = Reduce(state, ActionReceived{Action: PhotoSendSubmit, At: time.Unix(100, 0)})
	return state, nil
}

func TestPhotoSendOpenCapturesActiveTopic(t *testing.T) {
	state := topicMediaBaseState(t)
	opened, _ := Reduce(state, ActionReceived{Action: OpenPhotoSend})
	...
	plain, _ = Reduce(plain, ActionReceived{Action: OpenPhotoSend})
...
func TestMediaSendSubmitCarriesTopicIDForVideoAudioDocument(t *testing.T) {
	tests := []struct {
		...
		{name: "video", path: "/v.mp4", want: func(c Command) bool { _, ok := c.(SendVideo); return ok }},
		{name: "audio", path: "/a.mp3", want: func(c Command) bool { _, ok := c.(SendAudio); return ok }},
		{name: "document", path: "/d.pdf", want: func(c Command) bool { _, ok := c.(SendDocument); return ok }},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			submitted, cmds := Reduce(state, ActionReceived{Action: PhotoSendSubmit, At: time.Unix(100, 0)})
...
func TestStickerPickerOpenCapturesTopicAndLoadCarriesTopic(t *testing.T) {
	...
	opened, cmds := Reduce(state, ActionReceived{Action: OpenStickerPicker})
...
func TestStickerSubmitCarriesTopicID(t *testing.T) {
	state := topicMediaBaseState(t)
	state.Focus = FocusComposer
	opened, _ := Reduce(state, ActionReceived{Action: OpenStickerPicker})
	state = opened
	state, _ = Reduce(state, StickersLoaded{RequestID: state.StickerPicker.RequestID, ChatID: 7, Stickers: []domain.StickerRef{{File: domain.MediaFileRef{ID: 3}}}})
	submitted, cmds := Reduce(state, ActionReceived{Action: StickerActivate, At: time.Unix(100, 0), StickerFileID: 3, RequestID: state.StickerPicker.RequestID})
	if len(cmds) != 1 {
		t.Fatalf("cmds = %#v", cmds)
	}
...
func TestMessageSearchTopicScopeOpenSubmitPaginate(t *testing.T) {
	...
	unchanged, commands := Reduce(noTopic, ActionReceived{Action: OpenMessageSearch})
	if unchanged.MessageSearch != nil || len(commands) != 0 {
		t.Fatalf("no-topic open = %#v %#v", unchanged.MessageSearch, commands)
	}

	submitted, commands := Reduce(opened, MessageSearchValueChanged{ChatID: 7, Value: "needle"})
	_, commands = Reduce(submitted, ActionReceived{Action: SubmitMessageSearch})
	if len(commands) != 1 {
...
func TestMessageSearchRejectsMismatchedTopicID(t *testing.T) {
	opened := topicSearchState(t)
	submitted, commands := Reduce(opened, MessageSearchValueChanged{ChatID: 7, Value: "needle"})
	submitted, _ = Reduce(submitted, ActionReceived{Action: SubmitMessageSearch})
	requestID := submitted.MessageSearch.RequestID
	_ = commands

	rejected, commands := Reduce(submitted, ChatMessagesSearched{
	...
	failed, commands := Reduce(submitted, ChatMessagesSearchFailed{RequestID: requestID, ChatID: 7, TopicID: 202})
	...
	loaded, _ := Reduce(submitted, ChatMessagesSearched{
	...
	activated, cmds := Reduce(loaded, ActionReceived{Action: Activate})
	if len(cmds) != 1 {
	...
	rejectedCtx, commands := Reduce(activated, SearchMessageContextLoaded{
	...
	matched, _ := Reduce(activated, SearchMessageContextLoaded{
...
func TestPinnedMessagesTopicScope(t *testing.T) {
	state := topicMediaBaseState(t)
	opened, cmds := Reduce(state, ActionReceived{Action: OpenPinnedMessages})
	...
	noTopic := topicMediaBaseState(t)
	delete(noTopic.SelectedTopics, 7)
	unchanged, commands := Reduce(noTopic, ActionReceived{Action: OpenPinnedMessages})
	...
	rejected, commands := Reduce(opened, PinnedMessagesLoaded{
		RequestID: requestID, ChatID: 7, TopicID: 202,
		Page: telegram.MessageSearchPage{Messages: []domain.Message{{ID: 80, ChatID: 7, TopicID: 202}}},
	})
	...
	rejectedFailed, commands := Reduce(opened, PinnedMessagesLoadFailed{RequestID: requestID, ChatID: 7, TopicID: 202})
	...
	accepted, _ := Reduce(opened, PinnedMessagesLoaded{
		RequestID: requestID, ChatID: 7, TopicID: 101,
		Page: telegram.MessageSearchPage{Messages: []domain.Message{{ID: 80, ChatID: 7, TopicID: 101}}},
	})
	...
	activated, cmds := Reduce(accepted, ActionReceived{Action: Activate})
	if len(cmds) != 1 {
		t.Fatalf("activate cmds = %#v", cmds)
	}
	...
	// Context loaded with mismatched TopicID is rejected.
	rejectedCtx, commands := Reduce(activated, PinnedMessageContextLoaded{
		RequestID: jump.RequestID, ChatID: 7, TopicID: 202, MessageID: 80,
		Page: telegram.MessagePage{Messages: []domain.Message{{ID: 80, ChatID: 7, TopicID: 202}}},
	})
...
func TestReconcilePinnedMessagesIgnoresCrossTopicPinEvents(t *testing.T) {
	...
	got, _ := Reduce(state, TelegramEvent{Value: telegram.MessagePinnedUpdated{ChatID: 7, MessageID: 300, Pinned: true}})
	...
	unpinned, _ := Reduce(state, TelegramEvent{Value: telegram.MessagePinnedUpdated{ChatID: 7, MessageID: 200, Pinned: false}})
