...
func openChatSettings(state State) (State, []Command) {
	...
	return state, []Command{LoadChatSettingsCommand{RequestID: id, ChatID: chat.ID}}
}

func beginChatSettingEditor(state State, field ChatSettingField) (State, []Command) {
...
func chatSettingSave(state State) (State, []Command) {
	s := state.ChatSettings
	...
	s.Notice = ""
	return state, []Command{SaveChatSettingCommand{RequestID: id, ChatID: s.ChatID, Field: s.PendingField, Value: value}}
}
func reduceChatSettingsAction(state State, e ActionReceived) (State, []Command) {
	s := state.ChatSettings
	...
	if s.Error != nil {
		if e.Action == Retry || e.Action == Activate {
			...
			s.Loading = true
			s.Error = nil
			return state, []Command{LoadChatSettingsCommand{RequestID: id, ChatID: s.ChatID}}
		}
		return state, nil
	}
	...
	case SelectChatSlowMode:
		...
		s.Working = true
		return state, []Command{SaveChatSettingCommand{RequestID: id, ChatID: s.ChatID, Field: ChatSettingSlowMode, Delay: e.SlowModeDelay}}
...
func reduceChatSettingsLoaded(state State, e ChatSettingsLoaded) (State, []Command) {
	s := state.ChatSettings
...
func reduceChatSettingsLoadFailed(state State, e ChatSettingsLoadFailed) (State, []Command) {
	s := state.ChatSettings
...
func reduceChatSettingSaved(state State, e ChatSettingSaved) (State, []Command) {
	s := state.ChatSettings
...
func reduceChatSettingSaveFailed(state State, e ChatSettingSaveFailed) (State, []Command) {
	s := state.ChatSettings
	if s != nil && chatSettingsActive(state, e.ChatID) && s.RequestID == e.RequestID && s.ChatID == e.ChatID && s.PendingField == e.Field && s.Working {
		s.Working = false
		switch e.Field {
