...
func TestHandlerBootstrapCreatesOnceAndStreamsOrderedUpdates(t *testing.T) {
	...
	handler.Handle(ctx, LoadBootstrap{}, func(event Event) { events <- event })
	first := receiveWithTimeout(t, events, "ready update")
	second := receiveWithTimeout(t, events, "connection update")
	...
	handler.Handle(ctx, LoadBootstrap{}, func(Event) {})
	if factoryCalls != 1 {
...
func TestHandlerSubmitPromptPassesExactResponse(t *testing.T) {
	...
	handler.Handle(context.Background(), SubmitPrompt{Response: auth.Response{PromptID: prompt.ID, Value: "exact response"}}, func(Event) {})
...
func TestHandlerBootstrapForwardsBrokerPromptAndExactSubmission(t *testing.T) {
	...
	requested, ok := receiveWithTimeout(t, events, "forwarded prompt").(PromptRequested)
	if !ok || requested.Prompt.Kind != auth.PromptCode || requested.Prompt.ID == 0 {
		t.Fatalf("prompt event = %#v", requested)
	}
	handler.Handle(ctx, SubmitPrompt{Response: auth.Response{PromptID: requested.Prompt.ID, Value: "02719"}}, func(event Event) {
		t.Fatalf("successful prompt submit emitted %#v", event)
	})
	if got := receiveWithTimeout(t, client.answer, "client prompt answer"); got != "02719" {
		t.Fatalf("client answer = %q", got)
	}
	if event, ok := receiveWithTimeout(t, events, "ready after prompt").(TelegramEvent); !ok {
...
func TestHandlerBootstrapForwardsResolverPromptBeforeCreatingClient(t *testing.T) {
	...
	requested, ok := receiveWithTimeout(t, events, "resolver prompt").(PromptRequested)
	if !ok || requested.Prompt.Kind != auth.PromptAPIID {
		t.Fatalf("resolver prompt event = %#v", requested)
	}
	handler.Handle(ctx, SubmitPrompt{Response: auth.Response{PromptID: requested.Prompt.ID, Value: "17001"}}, func(event Event) {
		t.Fatalf("successful resolver prompt submit emitted %#v", event)
	})
	...
	receiveWithTimeout(t, handleDone, "bootstrap handle return")
}
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if len(test.updates) > 0 {
				if _, ok := receiveWithTimeout(t, events, "ready").(TelegramEvent); !ok {
					t.Fatal("first event was not TelegramEvent")
				}
			}
			failure := receiveWithTimeout(t, events, "start failure")
			if test.wantStartup {
...
func TestHandlerShutdownNeverClosesBeforeStartBegins(t *testing.T) {
	...
	handler.Handle(ctx, BeginShutdown{}, func(Event) {})
	client.mu.Lock()
	...
	receiveWithTimeout(t, bootstrapDone, "bootstrap completion")
}
...
func TestHandlerShutdownCancelsAndClosesExactlyOnce(t *testing.T) {
	...
	handler.Handle(context.Background(), BeginShutdown{}, func(event Event) { events = append(events, event) })
	handler.Handle(context.Background(), BeginShutdown{}, func(event Event) { events = append(events, event) })
...
func TestHandlerForwardMessageFailureDoesNotEmitRawCause(t *testing.T) {
	...
	handler := &Handler{client: client, workCtx: context.Background(), now: time.Now}
	var got Event
	handler.Handle(context.Background(), ForwardMessageCommand{RequestID: 7, SourceChatID: 9, SourceMessageID: 2, DestinationChatID: 10}, func(event Event) { got = event })
...
func TestHandlerDeleteMessageFailureDoesNotEmitRawCause(t *testing.T) {
	...
	handler := &Handler{client: client, workCtx: context.Background(), now: time.Now}
	var got Event
	handler.Handle(context.Background(), DeleteMessageCommand{RequestID: 7, ChatID: 9, MessageID: 2, Revoke: true}, func(event Event) { got = event })
...
func TestHandlerPinMessageFailureDoesNotEmitRawCause(t *testing.T) {
	...
	handler := &Handler{client: client, workCtx: context.Background(), now: time.Now}
	var got Event
	handler.Handle(context.Background(), PinMessageCommand{RequestID: 7, ChatID: 9, MessageID: 2, Unpin: false}, func(event Event) { got = event })
...
func TestHandlerEditFailureDoesNotEmitRawCause(t *testing.T) {
	...
	handler := &Handler{client: client, workCtx: context.Background(), now: time.Now}
	var got Event
	handler.Handle(context.Background(), EditText{RequestID: 7, ChatID: 9, MessageID: 2, Text: "opaque-edit"}, func(event Event) { got = event })
...
func collectHandlerEvents(handler *Handler, command Command) []Event {
	events := make([]Event, 0, 1)
	handler.Handle(context.Background(), command, func(event Event) { events = append(events, event) })
	return events
}
...
func TestHandlerSendPhotoTransportFailureSanitized(t *testing.T) {
	...
	// Command fields are public; they must contain path/caption.
