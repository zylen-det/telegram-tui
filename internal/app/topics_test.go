...
func TestOpenTopicsGating(t *testing.T) {
	opened, commands := Reduce(topicsBaseState(t), ActionReceived{Action: OpenTopics})
	...
	unchanged, cmds := Reduce(nonForum, ActionReceived{Action: OpenTopics})
	...
	unchanged, cmds = Reduce(wrongFocus, ActionReceived{Action: OpenTopics})
	if unchanged.Topics != nil || len(cmds) != 0 {
	...
	opened, commands = Reduce(conversation, ActionReceived{Action: OpenTopics})
...
func TestTopicsCloseRestoresPreviousFocusAndKeepsSelectedTopics(t *testing.T) {
	...
	reduced, commands := Reduce(state, ActionReceived{Action: Close})
...
func TestTopicsSelectionClampsWithoutWrap(t *testing.T) {
	...
	reduced, commands := Reduce(last, ActionReceived{Action: SelectNext})
	...
	reduced, commands = Reduce(first, ActionReceived{Action: SelectPrevious})
...
func TestTopicsSelectNextOnLastEmitsNextPage(t *testing.T) {
	...
	reduced, commands := Reduce(state, ActionReceived{Action: SelectNext})
...
func TestTopicsLoadedIgnoresStaleResults(t *testing.T) {
	state := topicsBaseState(t)
	reduced, _ := Reduce(state, ActionReceived{Action: OpenTopics})
	page := telegram.TopicPage{Topics: []domain.ForumTopic{testTopic(101, "A")}, TotalCount: 1}
	reduced, commands := Reduce(reduced, TopicsLoaded{RequestID: 99, ChatID: 7, Page: page})
	...
	reduced, commands = Reduce(reduced, TopicsLoaded{RequestID: 10, ChatID: 8, Page: page})
	if len(reduced.Topics.Results) != 0 || !reduced.Topics.Loading || len(commands) != 0 {
...
func TestTopicsLoadedDeduplicatesInPlaceAndAppendsNew(t *testing.T) {
	...
	reduced, commands := Reduce(reduced, TopicsLoaded{RequestID: 10, ChatID: 7, Page: page})
	got := reduced.Topics
...
func TestTopicsLoadFailedSetsSanitizedError(t *testing.T) {
	state := topicsBaseState(t)
	reduced, _ := Reduce(state, ActionReceived{Action: OpenTopics})
	reduced, commands := Reduce(reduced, TopicsLoadFailed{RequestID: 10, ChatID: 7, Error: domain.AppError{Kind: domain.ErrorNetwork, Op: "load topics", Message: "boom"}})
	...
	stale, _ := Reduce(reduced, TopicsLoadFailed{RequestID: 99, ChatID: 7, Error: domain.AppError{Message: "stale"}})
...
func TestSelectTopicValidatesChatAndRow(t *testing.T) {
	...
	wrongChat, commands := Reduce(state, ActionReceived{Action: SelectTopic, ChatID: 8, TopicID: 101})
	if wrongChat.Topics == nil || len(commands) != 0 {
		t.Fatalf("wrong chat: topics=%#v commands=%#v", wrongChat.Topics, commands)
	}
	unknown, commands := Reduce(state, ActionReceived{Action: SelectTopic, ChatID: 7, TopicID: 999})
	...
	unchanged, commands := Reduce(plain, ActionReceived{Action: SelectTopic, ChatID: 7, TopicID: 101})
...
func TestActivateTopicSeedsDraftsAndRequestsTopicMessages(t *testing.T) {
	...
	reduced, commands = Reduce(reduced, ActionReceived{Action: SelectTopic, ChatID: 7, TopicID: 101})
...
func TestTopicMessagesLoadedUsesTopicHistoryOnly(t *testing.T) {
	...
	stale, staleCommands := Reduce(state, MessagesLoaded{RequestID: 12, ChatID: 7, TopicID: 101, Page: telegram.MessagePage{Messages: []domain.Message{{ID: 2, ChatID: 7, TopicID: 101, Kind: domain.MessageText, Text: "b"}}}})
	...
	reduced, commands := Reduce(state, MessagesLoaded{
		RequestID: 10,
		ChatID:    7,
		TopicID:   101,
...
func TestTopicMessagesLoadFailedUsesTopicHistoryOnly(t *testing.T) {
	...
	state.SelectedTopics[7] = 101
	state.TopicHistory[key] = HistoryState{Loading: true, RequestID: 10}
	reduced, _ := Reduce(state, MessagesLoadFailed{RequestID: 10, ChatID: 7, TopicID: 101, Error: domain.AppError{Message: "topic boom"}})
	if _, exists := reduced.History[7]; exists {
		t.Fatalf("chat history touched: %#v", reduced.History[7])
	}
	history := reduced.TopicHistory[key]
	...
	reduced, _ = Reduce(other, MessagesLoadFailed{RequestID: 10, ChatID: 7, TopicID: 101, Error: domain.AppError{Message: "topic boom"}})
...
func TestForumTopicInfoChangedMergesAllowlist(t *testing.T) {
	...
	reduced, _ := Reduce(newState(), TelegramEvent{Value: update})
	...
	reduced, _ = Reduce(newState(), TelegramEvent{Value: &ptr})
	...
	untouched, _ := Reduce(newState(), TelegramEvent{Value: telegram.ForumTopicInfoChanged{Topic: domain.ForumTopic{ID: 555, ChatID: 999, Name: "ghost"}}})
...
func TestForumTopicStateChangedMergesAllowlistAndGuardsDraft(t *testing.T) {
	...
	reduced, _ := Reduce(dirty, TelegramEvent{Value: update})
	entry := reduced.ForumTopics[7][101]
	...
	reduced, _ = Reduce(clean, TelegramEvent{Value: update})
	...
	reduced, _ = Reduce(clean, TelegramEvent{Value: &ptr})
...
func TestChatSwitchingEntersAllModeDirectly(t *testing.T) {
	...
	check := func(t *testing.T, name string, reduced State, commands []Command) {
		t.Helper()
	...
	reduced, commands := Reduce(base(), ActionReceived{Action: SelectChat, ChatID: 7})
	check(t, "select chat", reduced, commands)

	reduced, commands = Reduce(base(), ActionReceived{Action: SelectPrevious})
	check(t, "chat nav", reduced, commands)

	activateBase := base()
	activateBase.SelectedChat = 0
	menu, menuCommands := Reduce(activateBase, ActionReceived{Action: Activate})
	...
	reduced, commands = Reduce(menu, ActionReceived{Action: Activate})
	...
	reduced, commands = Reduce(nonForum, ActionReceived{Action: SelectChat, ChatID: 7})
	if reduced.Topics != nil || len(commands) == 0 {
