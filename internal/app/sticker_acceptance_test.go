...
func TestStickerPickerGuardsLoadingStaleErrorEmptyNavigationAndResize(t *testing.T) {
	...
	opened, commands := Reduce(base, ActionReceived{Action: OpenStickerPicker})
	...
	if !reflect.DeepEqual(commands, []Command{LoadStickers{RequestID: 10, ChatID: 9}}) {
		t.Fatalf("commands = %#v", commands)
	}
	stale, staleCommands := Reduce(opened, &StickersLoaded{RequestID: 9, ChatID: 9, Stickers: testStickers()})
	if !reflect.DeepEqual(stale, opened) || len(staleCommands) != 0 {
		t.Fatal("stale catalog mutated state")
	}
	loaded, commands := Reduce(opened, StickersLoaded{RequestID: 10, ChatID: 9, Stickers: testStickers()})
	...
	moved, _ := Reduce(loaded, ActionReceived{Action: StickerMoveRight})
	moved, _ = Reduce(moved, ActionReceived{Action: StickerMoveDown})
	if moved.StickerPicker.Selected != 2 {
		t.Fatalf("clamped navigation = %d", moved.StickerPicker.Selected)
	}
	resized, _ := Reduce(moved, Resized{Width: 60, Height: 18})
	...
	failed, _ := Reduce(opened, StickersLoadFailed{RequestID: 10, ChatID: 9, Error: domain.AppError{Kind: domain.ErrorNetwork, Message: "private"}})
	...
	empty, _ := Reduce(opened, StickersLoaded{RequestID: 10, ChatID: 9})
	...
	closed, _ := Reduce(failed, ActionReceived{Action: Close})
	...
	for _, mutate := range []func(*State){
		...
		mutate(&guarded)
		got, commands := Reduce(guarded, ActionReceived{Action: OpenStickerPicker})
...
func TestStickerPosterIntakeUsesThumbnailAcrossAllIngress(t *testing.T) {
	...
	for name, reduce := range map[string]func(State) (State, []Command){
		"history": func(state State) (State, []Command) {
			state.History[9] = HistoryState{RequestID: 5, Loading: true}
			return Reduce(state, MessagesLoaded{RequestID: 5, ChatID: 9, Page: telegram.MessagePage{Messages: []domain.Message{sticker}, Done: true}})
		},
		"upsert": func(state State) (State, []Command) {
			return Reduce(state, TelegramEvent{Value: telegram.MessageUpserted{Message: sticker}})
		},
		"content": func(state State) (State, []Command) {
			state.Messages[9] = []domain.Message{{ID: 77, ChatID: 9, Kind: domain.MessageText}}
			return Reduce(state, TelegramEvent{Value: telegram.MessageContentUpdated{ChatID: 9, MessageID: 77, Kind: domain.MessageSticker, Media: sticker.Media}})
		},
	} {
...
func TestStickerHandlerCatalogAndSendBoundaries(t *testing.T) {
	sticker := testStickers()[0]
	fake := telegram.NewFake(telegram.FakeData{Stickers: []domain.StickerRef{sticker}})
	handler := &Handler{client: fake, workCtx: context.Background(), now: func() time.Time { return time.Unix(50, 0) }}
	events := collectHandlerEvents(handler, LoadStickers{RequestID: 3, ChatID: 9})
	loaded, ok := events[0].(StickersLoaded)
	if !ok || loaded.RequestID != 3 || loaded.ChatID != 9 || !reflect.DeepEqual(loaded.Stickers, []domain.StickerRef{sticker}) {
		t.Fatalf("catalog event = %#v", events)
	}
	events = collectHandlerEvents(handler, SendSticker{RequestID: 4, LocalID: -1, ChatID: 9, Sticker: sticker, ReplyToMessageID: 44})
	queued, ok := events[0].(StickerQueued)
	want := telegram.SendStickerRequest{ChatID: 9, Sticker: sticker, ReplyToMessageID: 44}
	if !ok || queued.RequestID != 4 || queued.LocalID != -1 || queued.ChatID != 9 || !reflect.DeepEqual(fake.StickerSendCalls(), []telegram.SendStickerRequest{want}) {
		t.Fatalf("send event/calls = %#v/%#v", events, fake.StickerSendCalls())
	}
}

func TestStickerStaleQueueIDCollisionIsFullStateNoOp(t *testing.T) {
	state, _ := Reduce(stickerPickerState(), ActionReceived{Action: OpenStickerPicker})
	state, _ = Reduce(state, StickersLoaded{RequestID: 10, ChatID: 9, Stickers: testStickers()})
	sent, commands := Reduce(state, ActionReceived{Action: StickerActivate, RequestID: 10, StickerFileID: 101, At: time.Unix(100, 0)})
	...
	got, staleCommands := Reduce(sent, StickerQueued{
...
func TestStickerOptimisticSendQueueRetryAndTerminalReconciliation(t *testing.T) {
	...
	stale, _ := Reduce(sent, StickerQueued{RequestID: command.RequestID - 1, LocalID: -1, ChatID: 9, Message: domain.Message{ID: -20, Kind: domain.MessageSticker}})
	if !reflect.DeepEqual(stale, sent) {
		t.Fatal("stale queue result mutated state")
	}
	queued, _ := Reduce(sent, &StickerQueued{RequestID: command.RequestID, LocalID: -1, ChatID: 9, Message: domain.Message{ID: -20, ChatID: 9, Kind: domain.MessageSticker}})
	...
	failed, _ := Reduce(sent, StickerQueueFailed{RequestID: command.RequestID, LocalID: -1, ChatID: 9, Error: failure, FailedAt: time.Unix(101, 0)})
	retried, retryCommands := Reduce(failed, ActionReceived{Action: Retry, MessageID: -1, At: time.Unix(102, 0)})
	...
	got, invalidCommands := Reduce(invalid, ActionReceived{Action: Retry, MessageID: -1, At: time.Unix(102, 0)})
	if !reflect.DeepEqual(got, before) || len(invalidCommands) != 0 {
		t.Fatal("invalid retry mutated state")
	}

	succeeded, _ := Reduce(sent, TelegramEvent{Value: telegram.MessageSendSucceeded{OldID: -1, Message: domain.Message{ID: 80, ChatID: 9, Kind: domain.MessageSticker, SendState: domain.SendSucceeded}}})
	...
	terminalFailed, _ := Reduce(sent, TelegramEvent{ReceivedAt: time.Unix(103, 0), Value: telegram.MessageSendFailed{OldID: -1, Message: domain.Message{ID: -2, ChatID: 9, Kind: domain.MessageSticker}, Error: failure}})
