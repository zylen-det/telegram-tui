...
func TestAdministrationPermissionToggleAndSave(t *testing.T) {
	...
	state, cmd := Reduce(state, ActionReceived{Action: OpenGroupPermissions})
	...
	state, _ = Reduce(state, AdministrationLoaded{RequestID: id, ChatID: 1, Snapshot: telegram.AdministrationSnapshot{Kind: domain.ChatSupergroup, CanRestrictMembers: true}})
	state, _ = Reduce(state, ActionReceived{Action: ToggleAdministrationItem, AdminIndex: 0})
	if !state.Administration.EditedPermissions.CanSendBasicMessages {
		t.Fatal("toggle failed")
	}
	state, cmd = Reduce(state, ActionReceived{Action: SaveAdministration})
...
func TestAdministrationStaleAndSanitizedFailure(t *testing.T) {
	state := State{Focus: FocusDetails, SelectedChat: 0, Chats: []domain.Chat{{ID: 1, Kind: domain.ChatSupergroup, CanRestrictMembers: true}}}
	state, _ = Reduce(state, ActionReceived{Action: OpenGroupPermissions})
	id := state.Administration.RequestID
	state, _ = Reduce(state, AdministrationLoadFailed{RequestID: id + 1, ChatID: 1})
	if state.Administration.Error != nil {
		t.Fatal("stale failure applied")
	}
	state, _ = Reduce(state, AdministrationLoadFailed{RequestID: id, ChatID: 1})
