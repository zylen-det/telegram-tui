...
func openGroupPermissions(state State) (State, []Command) {
	...
	return state, []Command{LoadAdministrationCommand{RequestID: id, ChatID: chat.ID}}
}

func openMemberAdministration(state State) (State, []Command) {
	...
	state.Members = nil
	state.Focus = FocusAdministration
	return state, []Command{LoadAdministrationCommand{RequestID: id, ChatID: chatID}, LoadMemberAdministrationCommand{RequestID: id, ChatID: chatID, UserID: userID}}
}
...
func reduceAdministrationAction(state State, e ActionReceived) (State, []Command) {
	...
	if e.Action == Retry && s.Error != nil {
		...
		s.MemberStatus = nil
		s.Selected = 0
		commands := []Command{LoadAdministrationCommand{RequestID: requestID, ChatID: s.ChatID}}
		if s.UserID != 0 {
	...
	case SaveAdministration:
		...
		s.Working = true
		if s.Mode == AdministrationDefaultPermissions {
			return state, []Command{SetDefaultChatPermissionsCommand{RequestID: id, ChatID: s.ChatID, Permissions: s.EditedPermissions}}
		}
		...
		return state, []Command{ApplyMemberAdministrationCommand{RequestID: id, Request: telegram.MemberAdministrationRequest{ChatID: s.ChatID, UserID: s.UserID, Action: action, Rights: s.EditedRights, Permissions: s.EditedPermissions}}}
	case ConfirmAdministrationAction:
		...
		s.RequestID = id
		s.Working = true
		return state, []Command{ApplyMemberAdministrationCommand{RequestID: id, Request: telegram.MemberAdministrationRequest{ChatID: s.ChatID, UserID: s.UserID, Action: s.PendingAction}}}
	}
	return state, nil
}
...
func reduceAdministrationLoaded(state State, e AdministrationLoaded) (State, []Command) {
	s := state.Administration
...
func reduceMemberAdministrationLoaded(state State, e MemberAdministrationLoaded) (State, []Command) {
	s := state.Administration
